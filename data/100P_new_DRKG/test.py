...
def dist_count():
    ...
    with open('all_data.txt') as f:
        for i in f:
            ...
            data_dict[tg[0]].append(i)
            data_dict[tg[2]].append(i)

    
    x = []
    for i in data_dict:
        x.append(len(data_dict[i]))
    ...
    # 使用 unique 函数获取唯一元素和其对应的出现次数
    unique_elements, counts = np.unique(arr, return_counts=True)

    count_dict = {} #存放每一个值出现的频率
    for i, j in zip(unique_elements, counts):
        count_dict[i] = j

    #对字典排序
    count_dict = dict(sorted(count_dict.items(), reverse = True, key=lambda item: item[1]))

    # 将结果打印出来
    for k in list(count_dict.keys())[:10]:
    ...
    #sns.distplot(data,bins=30,kde_kws={"color":"seagreen", "lw":3 }, hist_kws={ "color": "r" })  #直方图

    #sns.distplot(x, ax=axes[1, i], bins=30, label = model, kde_kws={"color":"seagreen", "lw":3 }, hist_kws={ "color": "r" })  #直方图
    #sns.distplot(x, kde_kws={"lw":2}, hist = True, kde = True)
    sns.histplot(x, ax=ax, kde = True)
    ax.set_xlim(0, 20)
    ax.set_ylim(0, 3000)

    plt.savefig('node_hist.png', bbox_inches = 'tight') #dpi = 400
    #plt.show()
    plt.close() 
