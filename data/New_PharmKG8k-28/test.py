...
def count4():
    ...
    for file in file_list[:]:
        ...
        print(f'{file}去重复后的总数据:',len(count_set))  
...
def count6():
    ...
    rdf={}
    classes={}
    with open('new_train_class.txt')as f:
        for i in tqdm(f):
            tg=i.rstrip('\n').split('\t')
            node1[tg[0]]=tg[0]
            #node1[tg[2]]=tg[2]
    ...
    with open('new_valid_class.txt')as f:
        for i in tqdm(f):
            tg=i.rstrip('\n').split('\t')
            node2[tg[0]]=tg[0]
            #node2[tg[2]]=tg[2]
    ...
    with open('new_test_class.txt')as f:
        for i in tqdm(f):
            tg=i.rstrip('\n').split('\t')
            node3[tg[0]]=tg[0]
            #node3[tg[2]]=tg[2]
...
def multi_relation():
    ...
    with open('all_triples.txt')as f:
        for i in f:
            tg = i.rstrip('\n').split('\t')
