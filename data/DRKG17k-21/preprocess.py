...
def gen_split(file_name):
    ...
    train, test, _, _ = train_test_split(df, df, test_size=0.11, random_state=2023)
    ...
    df.to_csv('sub_NoRepeat_drkg.txt',  index = False, header = False, sep = '\t')
    df.to_csv('sub_NoRepeat_drkg.tsv',  index = False, header = False, sep = '\t')
