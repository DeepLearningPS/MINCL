...
def gen_new_xhop3(batch_source_triples_1hop, unique_entities_train):
    ...
    new_1hop2 = {}
    for entity_u in tqdm(unique_entities_train):#遍历每一个训练实体。
        temp = []
        # print(i)
...
def gen_intra_context_edges(new_1hop): #new_1hop,每一个实体对应的正逆向三元组，字典的形式
    ...
    rel_dic2 = {}
    relation_num = 28
    for i in tqdm(range(relation_num)): #关系数量根据数据集而变
        #print(i)
        temp = []
...
def edge_index_gen(new_entity2id, new_1hop):
    edge_index = []
    count = 0
    for key, val in tqdm(new_entity2id.items()): #速度非常慢，程序卡在了这里？
        #print(count)
        count += 1
        ori = []
        dst = []
...
def batch_graph_gen(): #生成batchgraph_edgeindex.txt文件,该文件将在utils文件夹中的func.py文件中引用
    ...
    for index, edge in enumerate(edge_index):
        for i in range(len(edge[0])):
            rows.append(get_key(new_entity2id[index], edge[0][i])[0])#实际上就是节点对的第一列，转置之后变成了第一行
            cols.append(get_key(new_entity2id[index], edge[1][i])[0])#实际上就是节点对的第二列，转置之后变成了第二行
    print(len(rows))
    
    adj_edge = [rows, cols]
    np.savetxt("batchgraph_edgeindex.txt", adj_edge)#将节点对以np的方式保存成文本，此时文本的类型是浮点数的字符串而不是整型
...
if __name__ == '__main__':
    ...
    print('en_new_1hop')  #速度慢
    gen_new_1hop()
    print('gen_relation_triple')
    gen_relation_triple()
    print('gen_subgraph') #速度更慢
    gen_subgraph()
    
    print('batch_graph_gen')
    batch_graph_gen()
    count(file_list)
