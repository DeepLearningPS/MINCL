from helper import *
from data_loader import *
import sys
# sys.path.append('./')
from model.models_gat_no_dgi import *
from tqdm import tqdm
import os
import time
import pprint
#import tensorflow as tf
#多关系+对比

import torch.distributed as dist
import torch.multiprocessing as mp
from torch.nn.parallel import DistributedDataParallel as DDP

from collections import defaultdict
from ordered_set import OrderedSet

from functools import partial

import dill

#单机多卡
local_rank=int(os.environ["LOCAL_RANK"])
rank = int(os.environ["RANK"])
torch.cuda.set_device(local_rank)

class Runner(object):
    def load_data(self):
        """
        Reading in raw triples and converts it into a standard format.

        Parameters
        ----------
        self.p.dataset:         Takes in the name of the dataset (FB15k-237)

        Returns
        -------
        self.ent2id:            Entity to unique identifier mapping
        self.id2rel:            Inverse mapping of self.ent2id
        self.rel2id:            Relation to unique identifier mapping
        self.num_ent:           Number of entities in the Knowledge graph
        self.num_rel:           Number of relations in the Knowledge graph
        self.embed_dim:         Embedding dimension used
        self.data['train']:     Stores the triples corresponding to training dataset
        self.data['valid']:     Stores the triples corresponding to validation dataset
        self.data['test']:      Stores the triples corresponding to test dataset
        self.data_iter:		The dataloader for different data splits

        """

        ent_set, rel_set = OrderedSet(), OrderedSet()
        train_node_set = set()
        for split in ['train', 'valid', 'test']:
            for line in open('../data/{}/{}.txt'.format(self.p.dataset, split)):
                #sub, rel, obj = map(str.lower, line.strip().split('\t')) #不要区分大小写
                sub, rel, obj = line.strip().split('\t') #需要区分大小写
                ent_set.add(sub)
                rel_set.add(rel)
                ent_set.add(obj)
                if split == 'train':
                    train_node_set.add(sub)
                    train_node_set.add(obj)

        print('train node num:', len(train_node_set))
        print('rel_set num:', len(rel_set))
        self.ent2id = {ent: idx for idx, ent in enumerate(ent_set)}
        self.rel2id = {rel: idx for idx, rel in enumerate(rel_set)}
        self.rel2id.update({rel + '_reverse': idx + len(self.rel2id) for idx, rel in enumerate(rel_set)})

        self.id2ent = {idx: ent for ent, idx in self.ent2id.items()}
        self.id2rel = {idx: rel for rel, idx in self.rel2id.items()}
        self.p.id2ent = self.id2ent
        self.p.id2rel = self.id2rel

        self.p.num_ent = len(self.ent2id)
        self.p.num_rel = len(self.rel2id) // 2
        self.p.embed_dim = self.p.k_w * self.p.k_h if self.p.embed_dim is None else self.p.embed_dim

        self.data = ddict(list)
        sr2o = ddict(set)
        train_sr2o = ddict(set)

        if args.special_test != None: #use special test data
            for split in ['train', 'valid', 'test']:
                if split == 'test': #从指定的目录下加载特殊的测试集
                    for line in open('../data/{}/special_test/{}.txt'.format(self.p.dataset, args.special_test)):
                        #sub, rel, obj = map(str.lower, line.strip().split('\t'))
                        sub, rel, obj = line.strip().split('\t') #区分大小写
                        sub, rel, obj = self.ent2id[sub], self.rel2id[rel], self.ent2id[obj]
                        self.data[split].append((sub, rel, obj))
                else:       
                    for line in open('../data/{}/{}.txt'.format(self.p.dataset, split)):
                        #sub, rel, obj = map(str.lower, line.strip().split('\t'))
                        sub, rel, obj = line.strip().split('\t') #区分大小写
                        sub, rel, obj = self.ent2id[sub], self.rel2id[rel], self.ent2id[obj]
                        self.data[split].append((sub, rel, obj))

                        if split == 'train':
                            train_sr2o[(sub, rel)].add(obj)
                            train_sr2o[(obj, rel + self.p.num_rel)].add(sub)
        else:
            for split in ['train', 'valid', 'test']:
                for line in open('../data/{}/{}.txt'.format(self.p.dataset, split)):
                    #sub, rel, obj = map(str.lower, line.strip().split('\t'))
                    sub, rel, obj = line.strip().split('\t') #区分大小写
                    sub, rel, obj = self.ent2id[sub], self.rel2id[rel], self.ent2id[obj]
                    self.data[split].append((sub, rel, obj))

                    if split == 'train':
                        train_sr2o[(sub, rel)].add(obj)
                        train_sr2o[(obj, rel + self.p.num_rel)].add(sub)

        
        self.data = dict(self.data)

        self.train_sr2o = {k: list(v) for k, v in train_sr2o.items()}
        
        for split in ['train', 'valid', 'test']:
            for sub, rel, obj in self.data[split]:
                sr2o[(sub, rel)].add(obj)
                sr2o[(obj, rel + self.p.num_rel)].add(sub)

        self.sr2o_all = {k: list(v) for k, v in sr2o.items()}
        self.triples = ddict(list)
        
        
        #对比学习的数据结构，构造出来的训练集三元组的数量：self.triples['train']_num: 544230，而多标签分类的则是149689，显然数量增加3倍，因此，同样的批量速度慢了3倍。当然了，但凡按这种方式构造的
        #都会出现这种情况，这是正常的情况，只不过多标签分类自身的特性，导致了在构造训练集的时候可以将多个正的三元组合并一起，以此减少了数据量。因此，如果想赶上多标签分类，则可以选择使用更大的批量。
        #此时，如果想像多标签分类一样减少标签数量，则对比学习的正样本的需要从多个样本中随机采样一个或者全部使用，这要改写代码。实际结果表明，对比学习不适合这种数据结构       
        
        if self.p.loss_function == 'bcel_loss':
            for (sub, rel), obj in self.train_sr2o.items(): #多标签分类的数据构造
                self.triples['train'].append({'triple': (sub, rel, -1), 'label': self.train_sr2o[(sub, rel)], 'sub_samp': 1})
        else:
            for split in ['train']:  #自监督损失的数据构造
                for sub, rel, obj in self.data[split]:
                    rel_inv = rel + self.p.num_rel
                    self.triples['{}'.format(split)].append(
                        {'triple': (sub, rel, obj), 'label': self.train_sr2o[(sub, rel)], 'sub_samp': 1})
                    self.triples['{}'.format(split)].append(
                        {'triple': (obj, rel_inv, sub), 'label': self.train_sr2o[(obj, rel_inv)], 'sub_samp': 1})
        
        

        print("self.triples['train']_num:", len(self.triples['train'])) #149689,s elf.triples['train']_num: 544230 
        
         

        #for (sub, rel), obj in self.sr2o.items():
            #self.triples['train'].append({'triple': (sub, rel, -1), 'label': self.sr2o[(sub, rel)], 'sub_samp': 1})
            #self.triples['train'].append({'triple': (sub, rel, obj), 'label': self.sr2o[(sub, rel)], 'sub_samp': 1})

        for split in ['test', 'valid']:
            for sub, rel, obj in self.data[split]:
                rel_inv = rel + self.p.num_rel
                self.triples['{}_{}'.format(split, 'tail')].append(
                    {'triple': (sub, rel, obj), 'label': self.sr2o_all[(sub, rel)]})
                self.triples['{}_{}'.format(split, 'head')].append(
                    {'triple': (obj, rel_inv, sub), 'label': self.sr2o_all[(obj, rel_inv)]})

        self.triples = dict(self.triples)

        def get_data_loader(dataset_class, split, batch_size, shuffle=True):
            return DataLoader(
                dataset_class(self.triples[split], self.p),
                batch_size=batch_size,
                shuffle=shuffle,
                num_workers=max(0, self.p.num_workers),
                collate_fn=dataset_class.collate_fn
            )

        self.data_iter = {
            'train': get_data_loader(TrainDataset, 'train', self.p.batch_size),
            'valid_head': get_data_loader(TestDataset, 'valid_head', self.p.batch_size),
            'valid_tail': get_data_loader(TestDataset, 'valid_tail', self.p.batch_size),
            'test_head': get_data_loader(TestDataset, 'test_head', self.p.batch_size, shuffle=False), #因为要数据分析，所以顺序不要打乱，其它模型也改一下
            'test_tail': get_data_loader(TestDataset, 'test_tail', self.p.batch_size, shuffle=False),
        }

        self.edge_index, self.edge_type = self.construct_adj()

        # if self.p.pretrained_emb:
        if self.p.pretrained_emb:
            self.entity_embeddings, self.relation_embeddings = self.init_embeddings(
                os.path.join(self.p.dataset, 'entity2vec.txt'),
                os.path.join(self.p.dataset, 'relation2vec.txt'))
            print("Initialised relations and entities from TransE")
        else:
            self.entity_embeddings = np.random.randn(
                self.p.num_ent, 200)
            self.relation_embeddings = np.random.randn(
                self.p.num_rel, 200)
            print("Initialised relations and entities randomly")

    def init_embeddings(self, entity_file, relation_file):
        entity_emb, relation_emb = [], []

        with open('../data/' + entity_file) as f:
            for line in f:
                entity_emb.append([float(val) for val in line.strip().split()])

        with open('../data/' + relation_file) as f:
            for line in f:
                relation_emb.append([float(val) for val in line.strip().split()])

        return np.array(entity_emb, dtype=np.float32), np.array(relation_emb, dtype=np.float32)

    def construct_adj(self):
        """
        Constructor of the runner class

        Parameters
        ----------

        Returns
        -------
        Constructs the adjacency matrix for GCN

        """
        edge_index, edge_type = [], []

        for sub, rel, obj in self.data['train']:
            edge_index.append((sub, obj))
            edge_type.append(rel)

        # Adding inverse edges
        for sub, rel, obj in self.data['train']:
            edge_index.append((obj, sub))
            edge_type.append(rel + self.p.num_rel)

        edge_index = torch.LongTensor(edge_index).to(self.device).t()
        edge_type = torch.LongTensor(edge_type).to(self.device)

        return edge_index, edge_type

    def __init__(self, params):
        """
        Constructor of the runner class

        Parameters
        ----------
        params:         List of hyper-parameters of the model

        Returns
        -------
        Creates computational graph and optimizer

        """
        self.p = params
       
        self.logger = get_logger(args.task_name + '_'+ args.name + '.log', args.log_dir, args.config_dir)

        
        #self.logger = get_logger(args.task_name + '_' + args.name + '.log', args.workdir, args.config_dir)
        self.logger.setLevel(logging.DEBUG)
        
        
        #for arg in vars(args):
            #self.logger.info((arg, getattr(args, arg)))#注意使用logger.info打印数据的时候，一定加一个括号，将数据当元组来看
        #self.logger.info('\n\n')
        
        

        if self.p.gpu != '-1' and torch.cuda.is_available():
            #self.device = torch.device(f'cuda:{local_rank}')  #self.device
            self.device = torch.device('cuda')  #self.device
            torch.cuda.set_rng_state(torch.cuda.get_rng_state())
            torch.backends.cudnn.deterministic = True
        else:
            self.device = torch.device('cpu')

        self.load_data()  #self.load_model(save_path)
        self.model = self.add_model(self.p.score_func)
            
        
        
        #['CyclicLR', 'OneCycleLR', 'CosineAnnealingWarmRestarts', 'ReduceLROnPlateau', 'CosineAnnealingLR']
        if self.p.lr_scheduler == 'CosineAnnealingLR': #效果不错
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max = self.p.max_epochs)#余弦退火调整学习率
            #T_max更新学习率的迭代次数


        elif self.p.lr_scheduler == 'ReduceLROnPlateau': #动态衰减学习率 #效果不错
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode = 'min')
            #patience更新学习率的迭代次数

        elif self.p.lr_scheduler == 'CyclicLR': #CyclicLR循环学习率,stepsize=（样本个数/batchsize）*（2~10）,要放在batch里面
            self.scheduler = torch.optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=0.01, max_lr=0.1 )
        #因为cycle_momentum参数默认就是True而Adam优化器本身不带momentum参数的，所以就出现了如题的错误。
        #base_lr, max_lr,效果极差，不再测试，这个2个参数的值，设置大点比较好

        elif self.p.lr_scheduler == 'OneCycleLR': #total_steps:总的batch数,这个参数设置后就不用设置epochs和steps_per_epoch
            self.scheduler =torch.optim.lr_scheduler.OneCycleLR(self.optimizer, max_lr=0.01, \
                                                        steps_per_epoch = len(self.data_iter['train']), epochs = self.p.max_epochs)
        #max_lr 设置点比较好

        elif self.p.lr_scheduler == 'CosineAnnealingWarmRestarts': #T_mult = 2试试,效果不如T_mult = 1，暂时不再测试
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer, T_0 = 10, T_mult = 1)

        '''
        'CyclicLR','OneCycleLR','CosineAnnealingWarmRestarts'的参数设置是个难点。如果没把握或者不进行大量测试，建议使用上
        面两种方法
        
        '''

    def add_model(self, score_func):
        """
        Creates the computational graph

        Parameters
        ----------
        model_name:     Contains the model name to be created

        Returns
        -------
        Creates the computational graph for model and initializes it

        """
        #model_name = '{}_{}'.format(model, score_func)
        if score_func.lower() == 'transe':
            self.model = GAT_TransE(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        elif score_func.lower() == 'distmult':
            self.model = GAT_DistMult(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        elif score_func.lower() == 'conve':
           self. model = GAT_ConvE(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        elif score_func.lower() == 'simple':
            self.model = GAT_SimplE(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        elif score_func.lower() == 'complex':
            self.model = GAT_ComplEx(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        elif score_func.lower() == 'rotate':
            self.model = GAT_RotatE(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p) 
        elif score_func.lower() == 'rotatev2':
            self.model = GAT_RotatEv2(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p) 
        elif score_func.lower() == 'paire':
            self.model = GAT_PaiRE(self.device, 400, 0.2, 512, non_linearity='prelu', params=self.p)
        else:
            raise NotImplementedError
            
        if self.p.save_path != None:
            print('retrain')
            #多卡
            model = self.model.cuda(local_rank)
            model = DDP(model, [local_rank], find_unused_parameters=True)
            model = model.module
            self.optimizer = self.add_optimizer(model.parameters())
            self.load_model(os.path.join(self.p.model_dir, self.p.save_path))
            num_param = sum([p.numel() for p in self.model.parameters()])
            self.logger.info("Number of parameters: {0}".format(num_param)) #统计参数规模吗？可靠吗？怎么换算呢？等于多少M或者KB呢？
            
        else:
            print('train')
            #model.to(self.device)
            #多卡
            model = self.model.cuda(local_rank)
            model = DDP(model, [local_rank], find_unused_parameters=True)
            model = model.module
            self.optimizer = self.add_optimizer(model.parameters())
            num_param = sum([p.numel() for p in model.parameters()])
            self.logger.info("Number of parameters: {0}".format(num_param)) #统计参数规模吗？可靠吗？怎么换算呢？等于多少M或者KB呢？
            self.model = model
        
        
        return self.model

    def add_optimizer(self, parameters):
        """
        Creates an optimizer for training the parameters

        Parameters
        ----------
        parameters:         The parameters of the model

        Returns
        -------
        Returns an optimizer for learning the parameters of the model

        """
        if self.p.lr_scheduler != 'CyclicLR': #adamw与adam的唯一区别是，adamw是有默认权重衰减的，其它都一样。
            if args.optimizer == 'adamw':
                optimizer = torch.optim.AdamW(parameters)#AdamW的学习率会自动变化，学习率默认: 1e-3，权重衰减默认: 1e-2
            elif args.optimizer == 'adam':
                optimizer = torch.optim.Adam(parameters, lr=self.p.lr, weight_decay=self.p.l2)
                #optimizer = torch.optim.Adam(parameters, lr=args.lr, betas=(0.9, 0.99), eps=1e-05, weight_decay=args.l2) #不如默认的adam配置
                #Adam的学习率，如果weight_decay为0，则学习率不改变。默认为0。如果为0，则我们使用的各种学习学习率调整策略就无意义的
            elif args.optimizer == 'sgd':
                optimizer = torch.optim.SGD(parameters, lr=0.001)
            elif args.optimizer == 'adagrad':
                optimizer = torch.optim.Adagrad(parameters)
        else:
            optimizer = torch.optim.SGD(parameters, lr=0.001)
            
        return optimizer

    def read_batch(self, batch, split):
        """
        Function to read a batch of data and move the tensors in batch to CPU/GPU

        Parameters
        ----------
        batch: 		the batch to process
        split: (string) If split == 'train', 'valid' or 'test' split


        Returns
        -------
        Head, Relation, Tails, labels
        """
        if split == 'train':
            triple, label = [_.to(self.device) for _ in batch]
            return triple[:, 0], triple[:, 1], triple[:, 2], label
        else:
            triple, label = [_.to(self.device) for _ in batch]
            return triple[:, 0], triple[:, 1], triple[:, 2], label

    def save_model(self, save_path):
        """
        Function to save a model. It saves the model parameters, best validation scores,
        best epoch corresponding to best validation, state of the optimizer and all arguments for the run.

        Parameters
        ----------
        save_path: path where the model is saved

        Returns
        -------
        """
        state = {
            'state_dict': self.model.state_dict(),
            'best_val': self.best_val,
            'best_epoch': self.best_epoch,
            'optimizer': self.optimizer.state_dict(),
            'args'	: vars(self.p)
        }
        torch.save(state, save_path)



    def save_model_epoch(self, save_path, epoch):
        """
        Function to save a model. It saves the model parameters, best validation scores,
        best epoch corresponding to best validation, state of the optimizer and all arguments for the run.

        Parameters
        ----------
        save_path: path where the model is saved

        Returns
        -------
        """
        state = {
            'state_dict': self.model.state_dict(),
            'best_val': self.best_val,
            'best_epoch': self.best_epoch,
            'optimizer': self.optimizer.state_dict(),
            'args'	: vars(self.p)
        }
        torch.save(state, save_path)



    def load_model(self, load_path):
        """
        Function to load a saved model

        Parameters
        ----------
        load_path: path to the saved model

        Returns
        -------
        """
        state			= torch.load(load_path, map_location=f'cuda:{local_rank}')
        state_dict		= state['state_dict']
        
        self.best_val		= state['best_val']
        self.best_val_mrr	= self.best_val['mrr']

        #print('state_dict:', state_dict.keys())
        self.model.load_state_dict(state_dict)
        self.optimizer.load_state_dict(state['optimizer']) #只有在单卡的时候才能用
        #print("state['optimizer']:", state['optimizer'].keys()) #state['optimizer']: dict_keys(['state', 'param_groups'])
        self.optimizer.param_groups[0]['lr'] = state['optimizer']['param_groups'][0]['lr']



    def evaluate(self, split):
        """
        Function to evaluate the model on validation or test set

        Parameters
        ----------
        split: (string) If split == 'valid' then evaluate on the validation set, else the test set
        epoch: (int) Current epoch count

        Returns
        -------
        resutls:			The evaluation results containing the following:
            results['mr']:         	Average of ranks_left and ranks_right
            results['mrr']:         Mean Reciprocal Rank
            results['hits@k']:      Probability of getting the correct preodiction in top-k ranks based on predicted score

        """
        if split == 'valid':
            left_results,left_loss = self.predict(split=split, mode='tail_batch')
            right_results,right_loss = self.predict(split=split, mode='head_batch')
            valid_loss = (left_loss + right_loss) / 2
            results = get_combined_results(left_results, right_results)
            #logger.info(
                #'[Epoch {} {}]: MRR: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(epoch, split, results['left_mrr'],
                                                                        #results['right_mrr'], results['mrr']))
            return results,valid_loss
        else:
            left_results = self.predict(split=split, mode='tail_batch')
            right_results = self.predict(split=split, mode='head_batch')
            results = get_combined_results(left_results, right_results)
            return results
        



    def predict(self, split='valid', mode='tail_batch'):
        """
        Function to run model evaluation for a given mode

        Parameters
        ----------
        split: (string) 	If split == 'valid' then evaluate on the validation set, else the test set
        mode: (string):		Can be 'head_batch' or 'tail_batch'

        Returns
        -------
        resutls:			The evaluation results containing the following:
            results['mr']:         	Average of ranks_left and ranks_right
            results['mrr']:         Mean Reciprocal Rank
            results['hits@k']:      Probability of getting the correct preodiction in top-k ranks based on predicted score

        """
        self.model.eval()

        with torch.no_grad():
            results = {}
            losses = []
            train_iter = iter(self.data_iter['{}_{}'.format(split, mode.split('_')[0])])

            for step, batch in enumerate(train_iter):

                if split == 'valid':
                    sub, rel, obj, label = self.read_batch(batch, 'valid')
                    pred, cl_loss = self.model.forward(sub, rel)
                    #tri_loss = self.model.loss(pred, label)

                    if self.p.loss_function == 'contrastive_loss':
                        tri_loss = self.contrastive_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
                    elif self.p.loss_function == 'mask_softmax':
                        tri_loss = self.mask_softmax(pred, label, obj)
                    elif self.p.loss_function == 'mix_loss':
                        tri_loss = self.mix_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
                    elif self.p.loss_function == 'bcel_loss':
                        tri_loss = self.bcel_loss(pred, label)#多标签分类任务，所以label是矩阵的形式，而非向量，不同于二元分类和多元分类时传递真值的下标向量
                    elif self.p.loss_function == 'magr_loss':
                        tri_loss = self.magr_loss(pred, label, obj)

                    loss = tri_loss + self.p.subgraph_loss_rate * cl_loss
                    losses.append(loss.item())
                else:
                    sub, rel, obj, label = self.read_batch(batch, split)
                    pred, cl_loss = self.model.forward(sub, rel)
                
                b_range = torch.arange(pred.size()[0], device=self.device)
                target_pred = pred[b_range, obj]
                pred = torch.where(label.byte(), -torch.ones_like(pred) * 10000000, pred)
                pred[b_range, obj] = target_pred
                ranks = 1 + torch.argsort(torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)[
                    b_range, obj]

                ranks = ranks.float()
                results['count'] = torch.numel(ranks) + results.get('count', 0.0)
                results['mr'] = torch.sum(ranks).item() + results.get('mr', 0.0)
                results['mrr'] = torch.sum(1.0 / ranks).item() + results.get('mrr', 0.0)
                for k in range(10):
                    results['hits@{}'.format(k + 1)] = torch.numel(ranks[ranks <= (k + 1)]) + results.get(
                        'hits@{}'.format(k + 1), 0.0)

                #if step % 100 == 0:
                    #logger.info('[{}, {} Step {}]\t{}'.format(split.title(), mode.title(), step, self.p.name))
                    #title()首字母大写,这里的输出没意义
            if split == 'valid':
                valid_loss = np.mean(losses)
                return results,valid_loss
            else:
                return results



    def run_epoch(self, epoch, val_mrr = 0):
        """
        Function to run one epoch of training

        Parameters
        ----------
        epoch: current epoch count

        Returns
        -------
        loss: The loss value after the completion of one epoch
        """
        #change_lr = True #是否动态更改学习率
        self.model.train()
        losses = []
        train_iter = iter(self.data_iter['train'])
        iters = len(self.data_iter['train'])
        b_xent = nn.BCEWithLogitsLoss() #未使用
        lbl = torch.cat((torch.ones(1, self.p.num_ent), torch.zeros(1, self.p.num_ent)), 1).cuda() #未使用
        #print('batch num:', len(train_iter))
        for step, batch in enumerate(train_iter):
            self.optimizer.zero_grad()
            sub, rel, obj, label = self.read_batch(batch, 'train')
            pred, cl_loss = self.model.forward(sub, rel)
            #tri_loss = self.model.loss(pred, label)

            if self.p.loss_function == 'contrastive_loss':
                tri_loss = self.contrastive_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
            elif self.p.loss_function == 'mask_softmax':
                tri_loss = self.mask_softmax(pred, label, obj)
            elif self.p.loss_function == 'mix_loss':
                tri_loss = self.mix_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
            elif self.p.loss_function == 'bcel_loss':
                tri_loss = self.bcel_loss(pred, label)#多标签分类任务，所以label是矩阵的形式，而非向量，不同于二元分类和多元分类时传递真值的下标向量
            elif self.p.loss_function == 'magr_loss':
                tri_loss = self.magr_loss(pred, label, obj)

            loss = tri_loss + self.p.subgraph_loss_rate * cl_loss

            if args.change_lr:
                #新的优化器，学习率自动变化
                #self.optimizer.zero_grad() 
                torch.cuda.empty_cache()
                #loss.backward(retain_graph=True)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), args.clip)
                self.optimizer.step()
                
                if self.p.lr_scheduler in ['CyclicLR','OneCycleLR']:
                    self.scheduler.step()
                elif self.p.lr_scheduler == 'CosineAnnealingWarmRestarts':
                    self.scheduler.step(epoch - 1 + step / iters)
                
            else:
                loss.backward()
                self.optimizer.step()

            losses.append(loss.item())
            #print('batch id:', step)
    
        loss = np.mean(losses)
        return loss




    def run_epoch_old(self, epoch, val_mrr=0):#未使用
        """
        Function to run one epoch of training

        Parameters
        ----------
        epoch: current epoch count

        Returns
        -------
        loss: The loss value after the completion of one epoch
        """
        self.model.train()
        losses = []
        train_iter = iter(self.data_iter['train'])
        iters = len(self.data_iter['train'])#获取批量数量，控制学习率的步长变化
        
        for step, batch in enumerate(train_iter):
            #print('trian')
            self.optimizer.zero_grad()
            sub, rel, obj, label = self.read_batch(batch, 'train')
            x, r, y = self.gnn.forward(sub, rel)
            pred = self.matcher.forward(x, r, y)

            if self.p.loss_function == 'contrastive_loss':
                loss = self.contrastive_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
            elif self.p.loss_function == 'mask_softmax':
                loss = self.mask_softmax(pred, label, obj)
            elif self.p.loss_function == 'mix_loss':
                loss = self.mix_loss(pred, label, self.p.tau_plus, self.p.beta, self.p.estimator, obj)
            elif self.p.loss_function == 'bcel_loss':
                loss = self.bcel_loss(pred, label)#多标签分类任务，所以label是矩阵的形式，而非向量，不同于二元分类和多元分类时传递真值的下标向量
            elif self.p.loss_function == 'magr_loss':
                tri_loss = self.magr_loss(pred, label, obj)

            if args.change_lr:
                #新的优化器，学习率自动变化
                torch.cuda.empty_cache()
                #loss.backward(retain_graph=True)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), args.clip)
                self.optimizer.step()
                
                if self.p.lr_scheduler in ['CyclicLR','OneCycleLR']:
                    self.scheduler.step()
                elif self.p.lr_scheduler == 'CosineAnnealingWarmRestarts':
                    self.scheduler.step(epoch - 1 + step / iters)
            else:
                loss.backward()
                self.optimizer.step()

            losses.append(loss.item())
                
        
        loss = np.mean(losses)
        return loss




    def isinf_nan_if(self, x):
        if torch.isinf(x).any():
            #raise Exception(f'is inf:{x}')
            return 'inf'
        elif torch.isnan(x).any():
            #raise Exception(f'is nan:{x}')
            return 'nan'
        else:
            return 'success'
        

    def isinf_nan(self, x):
        if torch.isinf(x).any():
            raise Exception(f'is inf:{x}')
            #return 'inf'
        if torch.isnan(x).any():
            raise Exception(f'is nan:{x}')
            #return 'nan'
       


    def contrastive_loss(self, pred, label, tau_plus, beta, estimator, obj):
        st = time.time()

        try:
            if self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'nan':
                #pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0
                pred = pred / torch.abs(pred).max(dim = -1, keepdim = True)[0]
            #要判断输出是否含有inf或nan
            if (torch.exp(pred / self.p.temperature) == 0).any():
                pred = torch.exp(pred / self.p.temperature) + 1e-8 #防止下溢
            else:
                pred = torch.exp(pred / self.p.temperature)
            self.isinf_nan(pred)  
            
            loss = 0
            pos = []
            
            for i in range(len(label)):
                if args.lbl_smooth != 0:
                    index_line =  label[i] > 0.9
                else:
                    index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
                #pos_random_idx = random.sample(index_line.cpu().tolist(), 1)
                #pos_random_idx = index_line[0:1]
                if self.p.sample_mod == 'full':  #使用全部正样本，效果是不好的
                    #pos_random_idx = index_line
                    pos_random_idx = obj[i]
                elif self.p.sample_mod == 'part':
                    #pos_random_idx = index_line
                    #pos_random_idx = index_line[0:1]
                    #pos_random_idx = random.sample(index_line.cpu().tolist(), self.p.pos_sample_num)
                    pos_random_idx = obj[i] #obj存放的是每一个三元组的真实尾节点id，这里因为只取一个正样本，所以该样本来自于obj
                    #pos_random_idx = index_line #全取试试,全取效果不好
                else:
                    exit('please gain args.sample_mod')

                rs = pred[i][pos_random_idx].sum()#对每一行加和。根据经验，加法运算不合适，因为不同的样本，正样本的数量不同，如果使用加法则导致损失忽高忽低，难以有效指导模型
                #rs = pred[i][pos_random_idx].mean()
                #pos.append(rs.view(-1)) #shape = [1]
                pos.append(rs) #shape = [1]

            pos = torch.stack(pos, dim=0)#注意使用stack处理有梯度的张量列表时不会丢失梯度，其他链接方法，如直接将列表转化成张量，会丢失梯度的shape = [B,1] 

            
            if estimator=='hard':
                    N = LL-1
                    imp = (beta* neg.log()).exp()
                    reweight_neg = (imp*neg).sum(dim = -1) / imp.mean(dim = -1)
                    Ng = (-tau_plus * N * pos + reweight_neg) / (1 - tau_plus)
                    # constrain (optional)
                    Ng = torch.clamp(Ng, min = N * np.e**(-1 / args.temperature))
                    
            elif estimator == 'easy':
                neg = []
                index_line_bool = label <= 0
                for i in range(len(label)):
                    if args.lbl_smooth != 0: #如果标签平滑，则原来的0，现在 < 1，原来的1，现在 > 2
                        index_line =  label[i] < 0.1
                    else:
                        index_line = torch.nonzero(index_line_bool[i]).view(-1)
                    #neg_random_idx = random.sample(index_line.cpu().tolist(), 100) #减少负样本的数量，可以避免溢出
                    #neg_random_idx = index_line[0:100]
                    if self.p.sample_mod == 'full':
                        neg_random_idx = index_line
                        #neg_random_idx = np.random.permutation(index_line.cpu().numpy())#打乱数据，重新排列，并返回新的数据
                        
                        #index_line = index_line.cpu().numpy()
                        #np.random.shuffle(index_line) #速度更快，直接改变原数据
                        #neg_random_idx = index_line 
                    elif self.p.sample_mod == 'part':
                        #neg_random_idx = index_line[0:100]
                        neg_random_idx = random.sample(index_line.cpu().tolist(), self.p.neg_sample_num) 
                        #neg_random_idx = index_line #负样本全要
                    else:
                        exit('please gain args.sample_mod')

                    rs = pred[i][neg_random_idx].sum()
                    #rs = pred[i][neg_random_idx].mean()          
                    neg.append(rs)

                neg = torch.stack(neg, dim=0)         
            else:
                raise Exception('Invalid estimator selected. Please use any of [hard, easy]')

            if self.isinf_nan_if((- torch.log(pos / (pos + neg) )).mean()) == 'inf' or self.isinf_nan_if((- torch.log(pos / (pos + neg) )).mean()) == 'nan':
                pos = pos + 1e-8 #对数函数对接近0的数运算时，结果为-inf
                neg = neg + 1e-8        
            loss = (- torch.log(pos / (pos + args.Q * neg) )).mean()  #只有这里才可能出现inf之间的运算，进而出nan
            #loss = (- torch.log(pos / (pos + neg) )).sum()  #
            self.isinf_nan(loss)
        except Exception as e:
            print(e)
            #self.p.temperature = self.p.temperature + 0.1 #如果出现nan，则扩大温度系数
            #self.logger.info(f'temperature:{self.p.temperature}')
            exit('error')
            #continue
            
        en = time.time()
        #print('CL time:{}S'.format(en - st))    
        return loss








    def contrastive_loss_new(self, pred, label, tau_plus, beta, estimator, obj):
        st = time.time()
        
        #cp_pred = torch.full_like(pred, 1e-6)

    #except:
        if self.isinf_nan_if(pred / self.p.temperature) == 'nan':
            exit('pred is nan')
        
        elif self.isinf_nan_if(pred / self.p.temperature) == 'inf':
            exit('pred is inf')
        
        elif ((pred / self.p.temperature) == 0).any():
            exit('pred is 0')
            
        elif self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'nan':
            #pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0,这里更多的是上溢出问题
            pred = pred / torch.abs(pred).max(dim = -1, keepdim = True)[0] #这种缩放比pred / torch.abs(pred).max()好点
            #pred = pred / torch.abs(pred).max()
            pred = torch.exp(pred / self.p.temperature)
            #print('ok1')
            if (pred == 0).any():
                #print('ok2')
                count = pred == 0
                zero_index = torch.nonzero(count)
                for i in zero_index:
                    pred[i[0]][i[1]] += 1e-8
                
        
        elif (torch.exp(pred / self.p.temperature) == 0).any(): #这里可能出现下溢出，但如果统一加上一个小的数据有可能导致上溢出
            #print('ok3')
            pred = pred / (torch.abs(pred).max(dim = -1, keepdim = True)[0])
            #pred = pred / torch.abs(pred).max()
            pred = torch.exp(pred / self.p.temperature)  
            
            count = pred == 0
            #print('0 num:', count.sum())
            zero_index = torch.nonzero(count)
            #print('zero_index:', zero_index)
            
            for i in zero_index:
                pred[i[0]][i[1]] += 1e-8
            
            
            '''
            pred = pred / torch.abs(pred).max() #这里最容易上溢，因此，要缩放
            index = pred <= 0 #只让小的数值加系数
            alpha = torch.full_like(pred, 1e-6)
            alpha = torch.where(index == False, index, alpha)
            
            pred = torch.exp(pred / self.p.temperature + alpha) #防止下溢
            pred = torch.exp(pred / self.p.temperature + alpha)  #防止下溢
            #pred = pred / torch.abs(pred).max() #这里最容易上溢，因此，要缩放
            '''
        else:
            #print('ok4')
            #pred = pred / torch.abs(pred).max()
            #pred = torch.exp(pred / self.p.temperature)
            
            #pred = pred / torch.abs(pred).max() #加了缩放后，虽然不会出现溢出，但是效果不好，因为缩放后，各个值之间的差距非常小，得到了一组无差别的数据
            pred = pred / (torch.abs(pred).max(dim = -1, keepdim = True)[0]) #正常的情况下，不缩放，到最后可能出现nan，但是加了之后效果一般不如不加
            pred = torch.exp(pred / self.p.temperature)  
            
            count = pred == 0
            #print('0 num:', count.sum())
            zero_index = torch.nonzero(count)
            #print('zero_index:', zero_index)
            
            for i in zero_index:
                pred[i[0]][i[1]] += 1e-8
        
        if (pred < 0).any():
            print('pred2 exit < 0')
            exit('stop2 with < 0')
        
        '''
        if self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'nan':
            pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0
            #pred = torch.exp(pred / self.p.temperature) + 1e-5 #防止下溢
        #要判断输出是否含有inf或nan
        if (torch.exp(pred / self.p.temperature) == 0).any():
            pred = torch.exp(pred / self.p.temperature) + 1e-5 #防止下溢
        else:
            pred = torch.exp(pred / self.p.temperature)
        
        '''
        
        flag = 1
        self.isinf_nan(pred, flag)  
        
        loss = 0
        pos = []
            
        for i in range(len(label)):
            if args.lbl_smooth != 0:
                index_line =  label[i] > 0.9
            else:
                index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
            #pos_random_idx = random.sample(index_line.cpu().tolist(), 1)
            #pos_random_idx = index_line[0:1]
            if self.p.sample_mod == 'full':  #使用全部正样本，效果是不好的
                #pos_random_idx = index_line
                pos_random_idx = obj[i]
            elif self.p.sample_mod == 'part':
                #pos_random_idx = index_line
                #pos_random_idx = index_line[0:1]
                #pos_random_idx = random.sample(index_line.cpu().tolist(), self.p.pos_sample_num)
                pos_random_idx = obj[i] #obj存放的是每一个三元组的真实尾节点id，这里因为只取一个正样本，所以该样本来自于obj
                #pos_random_idx = index_line #全取试试,全取效果不好
            else:
                exit('please gain args.sample_mod')

            rs = pred[i][pos_random_idx].sum()#对每一行加和。根据经验，加法运算不合适，因为不同的样本，正样本的数量不同，如果使用加法则导致损失忽高忽低，难以有效指导模型
            #rs = pred[i][pos_random_idx].mean()
            #pos.append(rs.view(-1)) #shape = [1]
            pos.append(rs) #shape = [1]

        pos = torch.stack(pos, dim=0)#注意使用stack处理有梯度的张量列表时不会丢失梯度，其他链接方法，如直接将列表转化成张量，会丢失梯度的shape = [B,1] 

        if (pos < 0).any():
            print('pos exit < 0')
            exit('stop with < 0')
        
        if estimator=='hard':
                N = LL-1
                imp = (beta* neg.log()).exp()
                reweight_neg = (imp*neg).sum(dim = -1) / imp.mean(dim = -1)
                Ng = (-tau_plus * N * pos + reweight_neg) / (1 - tau_plus)
                # constrain (optional)
                Ng = torch.clamp(Ng, min = N * np.e**(-1 / args.temperature))
                
        elif estimator == 'easy':
            neg = []
            index_line_bool = label <= 0
            for i in range(len(label)):
                if args.lbl_smooth != 0: #如果标签平滑，则原来的0，现在 < 1，原来的1，现在 > 2
                    index_line =  label[i] < 0.1
                else:
                    index_line = torch.nonzero(index_line_bool[i]).view(-1)
                #neg_random_idx = random.sample(index_line.cpu().tolist(), 100) #减少负样本的数量，可以避免溢出
                #neg_random_idx = index_line[0:100]
                if self.p.sample_mod == 'full':
                    neg_random_idx = index_line
                    #neg_random_idx = np.random.permutation(index_line.cpu().numpy())#打乱数据，重新排列，并返回新的数据
                    
                    #index_line = index_line.cpu().numpy()
                    #np.random.shuffle(index_line) #速度更快，直接改变原数据
                    #neg_random_idx = index_line 
                elif self.p.sample_mod == 'part':
                    #neg_random_idx = index_line[0:100]
                    neg_random_idx = random.sample(index_line.cpu().tolist(), self.p.neg_sample_num) 
                    #neg_random_idx = index_line #负样本全要
                else:
                    exit('please gain args.sample_mod')

                rs = pred[i][neg_random_idx].sum()
                #rs = pred[i][neg_random_idx].mean()          
                neg.append(rs)

            neg = torch.stack(neg, dim=0)         
        else:
            raise Exception('Invalid estimator selected. Please use any of [hard, easy]')


        if (pos == 0).any():
            print('pos exit 0')
            exit('stop with 0')
        
        if self.isinf_nan_if(pos) == 'inf':
            print('pos exit inf')
            exit('stop with inf')
        
        if self.isinf_nan_if(pos) == 'nan':
            print('pos exit inf')
            exit('stop with nan')



        if (neg == 0).any():
            print('neg exit 0')
            exit('stop with 0')
        
        if self.isinf_nan_if(neg) == 'inf':
            print('neg exit inf')
            exit('stop with inf')
        
        if self.isinf_nan_if(neg) == 'nan':
            print('neg exit inf')
            exit('stop with nan')
        
        
        if self.isinf_nan_if((- torch.log(pos / (pos + neg) ))) == 'inf':
            #此时说明pos / (pos + neg)过大，使得取对数后上溢出，出现inf，解决办法缩放pos / (pos + neg)
         
            #print('1')
            tg = pos / (pos + neg)
            tg = tg / tg.abs().max(dim = -1, keepdim = True)[0]
            if (tg == 0).any():
                count = tg == 0
                zero_index = torch.nonzero(count)
                for i in zero_index:
                    tg[i] += 1e-8
                    
            loss = (- torch.log(tg)).mean()
        
        elif self.isinf_nan_if((- torch.log(pos / (pos + neg) ))) == 'nan':
            #此时说明pos / (pos + neg)自身就是inf，所以取对数后出现nan，可能除0所致，也可能pos太大所致，所以判断分母是否为0，再缩放pos / (pos + neg)
            #print('2')
            if ((pos + neg) == 0).any():
                print('21')
                tg = pos / ((pos + neg) + 1e-6)
                
            elif self.isinf_nan_if((pos / (pos + neg))) == 'inf':
                print('22')
                pos = pos / pos.abs().max(dim = -1, keepdim = True)[0]
                neg = neg / neg.abs().max(dim = -1, keepdim = True)[0]
                tg = pos / (pos + neg)
            
            elif self.isinf_nan_if((pos / (pos + neg))) == 'nan':
                print('23')
                pos = pos / pos.abs().max(dim = -1, keepdim = True)[0]
                neg = neg / neg.abs().max(dim = -1, keepdim = True)[0]
                tg = pos / (pos + neg)
            
            elif self.isinf_nan_if(torch.log(pos)) == 'nan': #inf - inf = nan； inf / inf = nan,只有这两种情况才出现nan。其余的inf运算是0或inf
                print('24') #执行了这一步  #exp和log操作inf后依旧是inf
                if self.isinf_nan_if(pos) == 'nan':
                    print('pos exit nan')
                    exit('stop with nan')
                
                if self.isinf_nan_if(pos) == 'inf': #前两个都不是导致nan的原因，因此可以判断pos存在负值
                    print('pos exit inf')
                    exit('stop with nan')
                
                else:
                    print('pos == - ?:', pos) #确实存在负数
                    print('pos.shape:', pos.shape)
                    
                
                pos = pos / pos.abs().max(dim = -1, keepdim = True)[0]
                neg = neg / neg.abs().max(dim = -1, keepdim = True)[0]
                tg = pos / (pos + neg)
                
            elif self.isinf_nan_if(torch.log(pos + neg)) == 'nan':
                print('25')
                pos = pos / pos.abs().max(dim = -1, keepdim = True)[0]
                neg = neg / neg.abs().max(dim = -1, keepdim = True)[0]
                tg = pos / (pos + neg)
                
            #elif self.isinf_nan_if((torch.log(pos / (pos + neg) ))) == 'nan': #torch在计算机分式对数时，还将除法变成减法运算，因此，检测pos / (pos + neg)是不行的
            elif self.isinf_nan_if((torch.log(pos) - torch.log(pos + neg))) == 'nan':
                print('26')
                pos = pos / pos.abs().max(dim = -1, keepdim = True)[0]
                neg = neg / neg.abs().max(dim = -1, keepdim = True)[0]
                tg = pos / (pos + neg)  
            else:
                print('27') #出现nan,但执行的这一步
                tg = pos / (pos + neg)
                loss = (- torch.log(tg)).mean()
                
                
        elif (- torch.log(pos / (pos + neg)) == 0).any():
            #print('3')
            tg = - torch.log(pos / (pos + neg))
            count = tg == 0
            zero_index = torch.nonzero(count)
            #print('zero_index:', zero_index)
            for i in zero_index:
                tg[i] += 1e-8
            loss = tg.mean()
            
            
        else:
            print('4')
            tg = - torch.log(pos / (pos + neg))
            flag = 10
            self.isinf_nan(tg, flag)
            loss = tg.mean()
            flag = 11
            self.isinf_nan(loss, flag)
            
        flag = 2
        self.isinf_nan(loss, flag)
    #except Exception as e:
        #print(e)
        #self.p.temperature = self.p.temperature + 0.1 #如果出现nan，则扩大温度系数
        #self.logger.info(f'temperature:{self.p.temperature}')
        #exit('error')
        #continue
            
        en = time.time()
        #print('CL time:{}S'.format(en - st))    
        return loss




    def contrastive_loss_test(self, pred, label, tau_plus, beta, estimator, obj):
        st = time.time()
        
        #cp_pred = torch.full_like(pred, 1e-6)

    #except:
        if self.isinf_nan_if(pred / self.p.temperature) == 'nan':
            exit('pred is nan')
        
        elif self.isinf_nan_if(pred / self.p.temperature) == 'inf':
            exit('pred is inf')
        
        elif ((pred / self.p.temperature) == 0).any():
            exit('pred is 0')
            
        elif self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'nan':
            #pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0,这里更多的是上溢出问题
            #pred = pred / torch.abs(pred).max(dim = -1, keepdim = True)[0]
            pred = pred / torch.abs(pred).max()
            pred = torch.exp(pred / self.p.temperature)
            print('ok1')
            if (pred == 0).any():
                print('ok2')
                count = pred == 0
                zero_index = torch.nonzero(count)
                for i in zero_index:
                    pred[i[0]][i[1]] += 1e-8
                
        
        elif (torch.exp(pred / self.p.temperature) == 0).any(): #这里可能出现下溢出，但如果统一加上一个小的数据有可能导致上溢出
                print('ok3')
                pred = pred / torch.abs(pred).max()
                pred = torch.exp(pred / self.p.temperature)  
                
                count = pred == 0
                #print('0 num:', count.sum())
                zero_index = torch.nonzero(count)
                #print('zero_index:', zero_index)
                
                for i in zero_index:
                    pred[i[0]][i[1]] += 1e-8
                
                
                '''
                pred = pred / torch.abs(pred).max() #这里最容易上溢，因此，要缩放
                index = pred <= 0 #只让小的数值加系数
                alpha = torch.full_like(pred, 1e-6)
                alpha = torch.where(index == False, index, alpha)
                
                pred = torch.exp(pred / self.p.temperature + alpha) #防止下溢
                pred = torch.exp(pred / self.p.temperature + alpha)  #防止下溢
                #pred = pred / torch.abs(pred).max() #这里最容易上溢，因此，要缩放
                '''
        else:
            print('ok4')
            #pred = pred / torch.abs(pred).max()
            #pred = torch.exp(pred / self.p.temperature)
            
            #pred = pred / torch.abs(pred).max() #加了缩放后，虽然不会出现溢出，但是效果不好，因为缩放后，各个值之间的差距非常小，得到了一组无差别的数据
            pred = pred / (torch.abs(pred).max(dim = -1, keepdim = True)[0])
            pred = torch.exp(pred / self.p.temperature)  
            
            count = pred == 0
            #print('0 num:', count.sum())
            zero_index = torch.nonzero(count)
            #print('zero_index:', zero_index)
            
            for i in zero_index:
                pred[i[0]][i[1]] += 1e-8
        
        if (pred < 0).any():
            print('pred2 exit < 0')
            exit('stop2 with < 0')
        
        '''
        if self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / self.p.temperature)) == 'nan':
            pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0
            #pred = torch.exp(pred / self.p.temperature) + 1e-5 #防止下溢
        #要判断输出是否含有inf或nan
        if (torch.exp(pred / self.p.temperature) == 0).any():
            pred = torch.exp(pred / self.p.temperature) + 1e-5 #防止下溢
        else:
            pred = torch.exp(pred / self.p.temperature)
        
        '''
        
        flag = 1
        self.isinf_nan(pred, flag)  
        
        loss = 0
        pos = []
            
        for i in range(len(label)):
            if args.lbl_smooth != 0:
                index_line =  label[i] > 0.9
            else:
                index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
            #pos_random_idx = random.sample(index_line.cpu().tolist(), 1)
            #pos_random_idx = index_line[0:1]
            if self.p.sample_mod == 'full':  #使用全部正样本，效果是不好的
                #pos_random_idx = index_line
                pos_random_idx = obj[i]
            elif self.p.sample_mod == 'part':
                #pos_random_idx = index_line
                #pos_random_idx = index_line[0:1]
                #pos_random_idx = random.sample(index_line.cpu().tolist(), self.p.pos_sample_num)
                pos_random_idx = obj[i] #obj存放的是每一个三元组的真实尾节点id，这里因为只取一个正样本，所以该样本来自于obj
                #pos_random_idx = index_line #全取试试,全取效果不好
            else:
                exit('please gain args.sample_mod')

            rs = pred[i][pos_random_idx].sum()#对每一行加和。根据经验，加法运算不合适，因为不同的样本，正样本的数量不同，如果使用加法则导致损失忽高忽低，难以有效指导模型
            #rs = pred[i][pos_random_idx].mean()
            #pos.append(rs.view(-1)) #shape = [1]
            pos.append(rs) #shape = [1]

        pos = torch.stack(pos, dim=0)#注意使用stack处理有梯度的张量列表时不会丢失梯度，其他链接方法，如直接将列表转化成张量，会丢失梯度的shape = [B,1] 

        if (pos < 0).any():
            print('pos exit < 0')
            exit('stop with < 0')
        
        if estimator=='hard':
                N = LL-1
                imp = (beta* neg.log()).exp()
                reweight_neg = (imp*neg).sum(dim = -1) / imp.mean(dim = -1)
                Ng = (-tau_plus * N * pos + reweight_neg) / (1 - tau_plus)
                # constrain (optional)
                Ng = torch.clamp(Ng, min = N * np.e**(-1 / args.temperature))
                
        elif estimator == 'easy':
            neg = []
            index_line_bool = label <= 0
            for i in range(len(label)):
                if args.lbl_smooth != 0: #如果标签平滑，则原来的0，现在 < 1，原来的1，现在 > 2
                    index_line =  label[i] < 0.1
                else:
                    index_line = torch.nonzero(index_line_bool[i]).view(-1)
                #neg_random_idx = random.sample(index_line.cpu().tolist(), 100) #减少负样本的数量，可以避免溢出
                #neg_random_idx = index_line[0:100]
                if self.p.sample_mod == 'full':
                    neg_random_idx = index_line
                    #neg_random_idx = np.random.permutation(index_line.cpu().numpy())#打乱数据，重新排列，并返回新的数据
                    
                    #index_line = index_line.cpu().numpy()
                    #np.random.shuffle(index_line) #速度更快，直接改变原数据
                    #neg_random_idx = index_line 
                elif self.p.sample_mod == 'part':
                    #neg_random_idx = index_line[0:100]
                    neg_random_idx = random.sample(index_line.cpu().tolist(), self.p.neg_sample_num) 
                    #neg_random_idx = index_line #负样本全要
                else:
                    exit('please gain args.sample_mod')

                rs = pred[i][neg_random_idx].sum()
                #rs = pred[i][neg_random_idx].mean()          
                neg.append(rs)

            neg = torch.stack(neg, dim=0)         
        else:
            raise Exception('Invalid estimator selected. Please use any of [hard, easy]')


        if (pos == 0).any():
            print('pos exit 0')
            exit('stop with 0')
        
        if self.isinf_nan_if(pos) == 'inf':
            print('pos exit inf')
            exit('stop with inf')
        
        if self.isinf_nan_if(pos) == 'nan':
            print('pos exit inf')
            exit('stop with nan')



        if (neg == 0).any():
            print('neg exit 0')
            exit('stop with 0')
        
        if self.isinf_nan_if(neg) == 'inf':
            print('neg exit inf')
            exit('stop with inf')
        
        if self.isinf_nan_if(neg) == 'nan':
            print('neg exit inf')
            exit('stop with nan')
        
        
        if self.isinf_nan_if((- torch.log(pos / (pos + neg) ))) == 'inf':
            #此时说明pos / (pos + neg)过大，使得取对数后上溢出，出现inf，解决办法缩放pos / (pos + neg)
         
            print('1')
            tg = pos / (pos + neg)
            tg = tg / tg.abs().max()
            if (tg == 0).any():
                count = tg == 0
                zero_index = torch.nonzero(count)
                for i in zero_index:
                    tg[i] += 1e-8
                    
            loss = (- torch.log(tg)).mean()
        
        elif self.isinf_nan_if((- torch.log(pos / (pos + neg) ))) == 'nan':
            #此时说明pos / (pos + neg)自身就是inf，所以取对数后出现nan，可能除0所致，也可能pos太大所致，所以判断分母是否为0，再缩放pos / (pos + neg)
            print('2')
            if ((pos + neg) == 0).any():
                print('21')
                tg = pos / ((pos + neg) + 1e-6)
                
            elif self.isinf_nan_if((pos / (pos + neg))) == 'inf':
                print('22')
                pos = pos / pos.abs().max()
                neg = neg / neg.abs().max()
                tg = pos / (pos + neg)
            
            elif self.isinf_nan_if((pos / (pos + neg))) == 'nan':
                print('23')
                pos = pos / pos.abs().max()
                neg = neg / neg.abs().max()
                tg = pos / (pos + neg)
            
            elif self.isinf_nan_if(torch.log(pos)) == 'nan': #inf - inf = nan； inf / inf = nan,只有这两种情况才出现nan。其余的inf运算是0或inf
                print('24') #执行了这一步  #exp和log操作inf后依旧是inf
                if self.isinf_nan_if(pos) == 'nan':
                    print('pos exit nan')
                    exit('stop with nan')
                
                if self.isinf_nan_if(pos) == 'inf': #前两个都不是导致nan的原因，因此可以判断pos存在负值
                    print('pos exit inf')
                    exit('stop with nan')
                
                else:
                    print('pos == - ?:', pos) #确实存在负数
                    print('pos.shape:', pos.shape)
                    
                
                pos = pos / pos.abs().max()
                neg = neg / neg.abs().max()
                tg = pos / (pos + neg)
                
            elif self.isinf_nan_if(torch.log(pos + neg)) == 'nan':
                print('25')
                pos = pos / pos.abs().max()
                neg = neg / neg.abs().max()
                tg = pos / (pos + neg)
                
            #elif self.isinf_nan_if((torch.log(pos / (pos + neg) ))) == 'nan': #torch在计算机分式对数时，还将除法变成减法运算，因此，检测pos / (pos + neg)是不行的
            elif self.isinf_nan_if((torch.log(pos) - torch.log(pos + neg))) == 'nan':
                print('26')
                pos = pos / pos.abs().max()
                neg = neg / neg.abs().max()
                tg = pos / (pos + neg)  
            else:
                print('27') #出现nan,但执行的这一步
                tg = pos / (pos + neg)
                loss = (- torch.log(tg)).mean()
                
                
        elif (- torch.log(pos / (pos + neg)) == 0).any():
            print('3')
            tg = - torch.log(pos / (pos + neg))
            count = tg == 0
            zero_index = torch.nonzero(count)
            #print('zero_index:', zero_index)
            for i in zero_index:
                tg[i] += 1e-8
            loss = tg.mean()
            
            
        else:
            print('4')
            tg = - torch.log(pos / (pos + neg))
            loss = tg.mean()
            
        flag = 2
        self.isinf_nan(loss, flag)
    #except Exception as e:
        #print(e)
        #self.p.temperature = self.p.temperature + 0.1 #如果出现nan，则扩大温度系数
        #self.logger.info(f'temperature:{self.p.temperature}')
        #exit('error')
        #continue
            
        en = time.time()
        #print('CL time:{}S'.format(en - st))    
        return loss








    def norm_data(self, score):
        score = score / self.p.embed_dim #  缩放

        score_norm  = torch.norm(score, p=2, dim=-1, keepdim=True)  #单位化
        score       = score / score_norm
        
        return score


    def mask_softmax_old(self, pred, label):
        #建议这里使用损失的均值，要不然初始损失有点大，影响mix_loss
        loss = 0
        pos = []
        #pred = pred / self.p.embed_dim
        pred_log = torch.log_softmax(pred, dim=-1) / self.p.embed_dim
        #pred_log = torch.log_softmax(pred, dim=-1)
        #softmax对缩放敏感，sigmoid也敏感，因为值缩小之后趋近与0，经过sigmoid的输出都接近于0.5， 而softmax的输出也接近一样于0.16，所以不能乱缩放
        #对pred求softmax,然后取每一行的真值，并相加，实际上是最大化正样本得分损失

        for i in range(len(label)):
            index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
            rs = pred_log[i][index_line].sum()#对每一行加和
            #rs = pred_log[i][index_line].mean()#对每一行求均值， 效果不如求和.一般情况下，同样的效果下，损失大一点效果更好
            pos.append(rs)

        loss = torch.stack(pos, dim=0).mean() #两者差异很小
        #loss = torch.stack(pos, dim=0).sum()
        return -loss
        


    def mask_softmax(self, pred, label, obj):
        #建议这里使用损失的均值，要不然初始损失有点大，影响mix_loss
        loss = 0
        pos = []
        #pred = pred / self.p.embed_dim
        pred_log = torch.log_softmax(pred, dim=-1) / self.p.embed_dim
        #pred_log = torch.log_softmax(pred, dim=-1)
        #softmax对缩放敏感，sigmoid也敏感，因为值缩小之后趋近与0，经过sigmoid的输出都接近于0.5， 而softmax的输出也接近一样于0.16，所以不能乱缩放
        #对pred求softmax,然后取每一行的真值，并相加，实际上是最大化正样本得分损失

        for i in range(len(label)):
            #index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
            index_line = obj[i]
            rs = pred_log[i][index_line].sum()#对每一行加和
            #rs = pred_log[i][index_line].mean()#对每一行求均值， 效果不如求和.一般情况下，同样的效果下，损失大一点效果更好
            pos.append(rs)

        loss = torch.stack(pos, dim=0).mean() #两者差异很小
        #loss = torch.stack(pos, dim=0).sum()
        return -loss
        

    def bcel_loss(self, pred, label):
        bceloss = torch.nn.BCELoss()#多标签分类
        pred_s = torch.sigmoid(pred)
        return bceloss(pred_s, label) 



    def magr_loss(self, pred, label, obj):
        '''
        对于包含N个样本的batch数据 D(x1,x2,y), x1,x2是给定的待排序的两个输入，y代表真实的标签，属于{1, −1}。
        当Y = 1 ，x1应该排在x2前，Y = −1 ，x1应该排在x2之后。
        '''
        #L = max(0, −y ∗ (x1 − x2) + margin) #margin默认是0， 当margin是0的时候，如果−y ∗ (x1 − x2)<0, 则表示排序正确，正样本排在了负样本后面，margin表示正样本比
        #负样本的得分高多少才算预测正确，即要出margin个数量
        magrloss= nn.MarginRankingLoss(0.5) #最大间隔损失

        #pred = torch.sigmoid(pred) #最大间隔类似于对比学习，因此这一步可能不是必要的，且可能降低模型性能
        #pred = torch.exp(pred)
        loss_list = []
        
        
        for i in range(len(label)):

            index_line_bool = label <= 0
            neg_random_idx = torch.nonzero(index_line_bool[i]).view(-1)
            n = pred[i][neg_random_idx] #一个向量
            #n = torch.norm(n, p=1, dim=-1, keepdim = True)
            #print('n.shape:', n.shape)
            #n = torch.stack(n, dim = 0)
            
            pos_random_idx = obj[i] 
            p = pred[i][[pos_random_idx]].repeat(n.size(0)) #一个向量，长度为1
            #p = torch.norm(p, p=1, dim=-1, keepdim = True)
            #p = torch.stack(p, dim = 0)
            
            mg = torch.ones(n.size(0)).cuda()
            #mg = -torch.ones(1).cuda()
            
        
            loss= magrloss(p, n, mg)
            loss_list.append(loss)
        
        loss = torch.stack(loss_list, dim = 0).mean()
        return loss
    
        
    def mix_loss(self, pred, label, tau_plus, beta, estimator, obj):

        '''
        将2种损失加在一起看看效果',结果是nan,梯度消失.现在有个问题：对比损失+多标签分类损失有问题，报错。但是单独跑是没问题的,
        出错的原因是对比损失为nan导致无法加法运算
        如果使用的混合损失，则需要两组预测值，显然，我们目前只有一组预测值，且没有经过sigmod，因此不适合BCELoss

        '''
        if args.mix_loss_mod == 'CLoss-MLoss':      #能运行
            all_loss = self.contrastive_loss(pred, label, tau_plus, beta, estimator, obj) + self.mask_softmax(pred, label, obj)
        elif args.mix_loss_mod == 'CLoss-BCELoss':  #能运行
            all_loss = self.contrastive_loss(pred, label, tau_plus, beta, estimator, obj) + self.bcel_loss(pred, label)
        elif args.mix_loss_mod == 'MLoss-BCELoss':  #能运行
            all_loss = self.mask_softmax(pred, label, obj) + self.bcel_loss(pred, label)
            
        #all_loss = self.p.loss_rate * self.contrastive_loss(pred, label, tau_plus, beta, estimator) + (1 - self.p.loss_rate) * self.bcel_loss(pred, label)
        #all_loss = self.p.loss_rate * contrastive_loss(pred, label, tau_plus, beta, estimator) + mask_softmax(pred, label)
        return all_loss




    

    def fit(self):
        """
        Function to run training and evaluation of model

        Parameters
        ----------

        Returns
        -------
        """
        best_val = 0
        best_epoch = 0
        best_lr = 0
        best_mrr = 0
        best_mr = 0
        best_hit1 = 0
        best_hit3 = 0
        best_hit10 = 0
        val_mrr = 0
        self.best_val_mrr, self.best_val, self.best_epoch = 0., {}, 0
        
        save_path = os.path.join(args.model_dir, args.task_name + '_' + args.name)#这个作为新模型的名字
        '''
        if self.p.restore:
            self.load_model(os.path.join(args.model_dir, args.save_path)) #如果是加载模型，则只需要传递一个新args.save_path

            self.logger.info('Successfully Loaded previous model')
        '''


        kill_cnt = 0
        for epoch in np.arange(self.p.max_epochs) + 1: 
            starting = time.time()      
            train_loss  = self.run_epoch(epoch, val_mrr)
                    
            if args.change_lr and self.p.lr_scheduler not in ['CyclicLR','OneCycleLR','CosineAnnealingWarmRestarts','ReduceLROnPlateau']:
                self.scheduler.step()
            
            val_results, valid_loss = self.evaluate('valid')
            
            if args.change_lr and self.p.lr_scheduler == 'ReduceLROnPlateau':  #需要在验证损失之后
                self.scheduler.step(valid_loss)
            
            ending = time.time()
            
            self.logger.info('验证:')
            self.logger.info(("Epoch:{}  time {}m  LR:{:.10f}").format(epoch, ((ending-starting)//60), self.optimizer.param_groups[0]['lr']))
            self.logger.info(("Train Loss:{:.10f}  Valid Loss:{:.10f}").format(train_loss,valid_loss))
            self.logger.info(("Valid MRR:{:.5f}  Valid MR:{:.5f}").format(val_results['mrr'], val_results['mr']))
            self.logger.info(("Valid H@1:{:.5f}  Valid H@3:{:.5f}  Valid H@10:{:.5f}").format(val_results['hits@1'],val_results['hits@3'],val_results['hits@10']))
            self.logger.info('\n\n')

            epoch_save_path = os.path.join(args.model_dir, 'epoch' + str(epoch))#这个作为新模型的名字
            self.save_model_epoch(epoch_save_path, epoch) #保存每一轮的模型

            if val_results['mrr'] > best_val:
                self.best_val = val_results
                self.best_val_mrr = val_results['mrr']
                self.best_epoch = epoch
                best_val = val_results['mrr']
                best_epoch = epoch
                best_mrr = val_results['mrr']
                best_mr = val_results['mr']
                best_hit1 = val_results['hits@1']
                best_hit3 = val_results['hits@3']
                best_hit10 = val_results['hits@10']
                best_lr = self.optimizer.param_groups[0]['lr']
                self.save_model(save_path)
                #torch.save(self.model, os.path.join(args.model_dir, args.task_name + '_' + args.name))#保存模型
                kill_cnt = 0

            else:
                kill_cnt += 1
                
                
                if kill_cnt % 10 == 0 and self.p.gamma > 5:
                    self.p.gamma -= 5
                    self.logger.info('Gamma decay on saturation, updated value of gamma: {}'.format(self.p.gamma))
                    self.logger.info('\n\n')
                
                
                
                if args.change_tem == 1:
                    if kill_cnt % 5 == 0 and self.p.save_path != None: #加载模型，温度系数需要递减，初始温度系数，一般是一个很小的值，如0.1,0.2
                        if round(self.p.temperature, 2) >= 1.49:
                            self.p.temperature = 0.1
                        else:
                            self.p.temperature += 0.1 #增加温度系数值,
                        self.logger.info('temperature add on saturation, updated value of temperature {}'.format(self.p.temperature))
                        self.logger.info('\n\n')
                        
                    if kill_cnt % 5 == 0 and self.p.save_path == None: #从头训练，温度系数需要递增，初始温度系数，一般是一个很大的值，如1.5,1.0
                        if round(self.p.temperature, 2) <= 0.1: #发现在衰减的过程中，会出现问题，变成浮点数，且有很长的小数位，因此需要先正则化，保留2位小数
                            self.p.temperature = 1.5
                        else:
                            self.p.temperature -= 0.1 #减少温度系数值,
                        self.logger.info('temperature decay on saturation, updated value of temperature {}'.format(self.p.temperature))
                        self.logger.info('\n\n')
                
                    
                if kill_cnt > args.stop_num:
                    self.logger.info("Early Stopping!!")
                    break
      

            if epoch % 5 ==0:
                self.logger.info(f'best_epoch: {best_epoch}')
                self.logger.info(('best_lr: %.10f') % (best_lr))
                self.logger.info(('best_mrr: %.5f') % (best_mrr))
                self.logger.info(('best_mr: %.5f') % (best_mr))
                self.logger.info(('best_hit1: %.5f') % (best_hit1))
                self.logger.info(('best_hit3: %.5f') % (best_hit3))
                self.logger.info(('best_hit10: %.5f') % (best_hit10))
                self.logger.info('\n\n')

        self.logger.info(f'best_epoch: {best_epoch}')
        self.logger.info(('best_lr: %.10f') % (best_lr))
        self.logger.info(('best_mrr: %.5f') % (best_mrr))
        self.logger.info(('best_mr: %.5f') % (best_mr))
        self.logger.info(('best_hit1: %.5f') % (best_hit1))
        self.logger.info(('best_hit3: %.5f') % (best_hit3))
        self.logger.info(('best_hit10: %.5f') % (best_hit10))
        self.logger.info('\n\n')
        
        #self.logger.info('Loading best model, Evaluating on Test data')
        self.load_model(save_path)
        #torch.load(os.path.join(args.model_dir, args.task_name + '_' + args.name))#加载模型
        test_results = self.evaluate('test')

        #self.logger.info('\n\n')
        self.logger.info('测试:')
        self.logger.info(('Test_mrr: %.5f') % test_results['mrr'])
        self.logger.info(('Test_mr: %.5f') % test_results['mr'])
        self.logger.info(('Test_hit1: %.5f') % test_results['hits@1'])
        self.logger.info(('Test_hit3: %.5f') % test_results['hits@3'])
        self.logger.info(('Test_hit10: %.5f') % test_results['hits@10'])

        localtime = time.asctime(time.localtime(time.time()))
        self.logger.info(f'程序终止时间:{localtime}')
        


    def test(self): #测试集可以单独拿来用，不用再调用fit()
        self.logger.info('Loading best model, Evaluating on Test data')
        save_path = os.path.join(args.model_dir, args.save_path)
        self.load_model(save_path)
        test_results = self.evaluate('test')

        self.logger.info('\n\n')
        self.logger.info('测试:')
        self.logger.info(('Test_mrr: %.5f') % test_results['mrr'])
        self.logger.info(('Test_mr: %.5f') % test_results['mr'])
        self.logger.info(('Test_hit1: %.5f') % test_results['hits@1'])
        self.logger.info(('Test_hit3: %.5f') % test_results['hits@3'])
        self.logger.info(('Test_hit10: %.5f') % test_results['hits@10'])

        localtime = time.asctime(time.localtime(time.time()))
        self.logger.info(f'程序终止时间:{localtime}')





    def transe(self, all_ent, all_rel, sub, rel):
        x_ln = torch.nn.LayerNorm(self.p.num_ent).cuda() #归一化，无参数
        sub_emb, rel_emb = all_ent[sub], all_rel[rel]
        obj_emb = sub_emb + rel_emb

        x = self.p.gamma - torch.norm(obj_emb.unsqueeze(1) - all_ent, p=1, dim=2)
        score = x_ln(x) 
        return score


    def distmult(self, all_ent, all_rel, sub, rel):
        x_ln = torch.nn.LayerNorm(self.p.num_ent).cuda() #归一化，无参数
        sub_emb, rel_emb = all_ent[sub], all_rel[rel]
        obj_emb = sub_emb * rel_emb

        x = torch.mm(obj_emb, all_ent.transpose(1, 0))
        score = x_ln(x) 
        return score


    def complex(self, all_ent, all_rel, sub, rel):
        x_ln = torch.nn.LayerNorm(self.p.num_ent).cuda() #归一化，无参数
        sub_emb, rel_emb = all_ent[sub], all_rel[rel]

        heads = sub_emb
        tails = all_ent
        rels  = rel_emb
        
        heads_re, heads_im = torch.chunk(heads, chunks=2, dim=-1)#分离实部与虚部
        tails_re, tails_im = torch.chunk(tails, chunks=2, dim=-1)
        rels_re, rels_im   = torch.chunk(rels, chunks=2, dim=-1)

        x = (
                 torch.mm(rels_re * heads_re, tails_re.transpose(1, 0)) +
                 torch.mm(rels_re * heads_im, tails_im.transpose(1, 0)) +
                 torch.mm(rels_im * heads_re, tails_im.transpose(1, 0))-
                 torch.mm(rels_im * heads_im, tails_re.transpose(1, 0))
                )

        score = x_ln(x) 
        return score
    

    def simple(self, all_ent, all_rel, sub, rel):
        x_ln = torch.nn.LayerNorm(self.p.num_ent).cuda() #归一化，无参数
        sub_emb, rel_emb = all_ent[sub], all_rel[rel]

        heads = sub_emb
        tails = all_ent
        rels  = rel_emb

        heads_h, heads_t = torch.chunk(heads, chunks=2, dim=-1)
        tails_h, tails_t = torch.chunk(tails, chunks=2, dim=-1)
        rel_a, rel_b = torch.chunk(rels, chunks=2, dim=-1)

        x = (
                 torch.mm(heads_h * rel_a, tails_t.transpose(1, 0)) +
                 torch.mm(heads_t * rel_b, tails_h.transpose(1, 0))
                ) / 2
    
        score = x_ln(x) 
        return score


    '''
    def isinf_nan_if(x):
        if torch.isinf(x).any():
            #raise Exception(f'is inf:{x}')
            return 'inf'
        elif torch.isnan(x).any():
            #raise Exception(f'is nan:{x}')
            return 'nan'
        else:
            return 'success'
        

    def isinf_nan(x):
        if torch.isinf(x).any():
            raise Exception(f'is inf:{x}')
            #return 'inf'
        if torch.isnan(x).any():
            raise Exception(f'is nan:{x}')
            #return 'nan'
    '''


    def analy_contrastive_loss(self, pred, label, obj, temperature = 1, sample_mod = 'full', neg_sample_num = 100):
        st = time.time()

        try:
            if self.isinf_nan_if(torch.exp(pred / temperature)) == 'inf' or self.isinf_nan_if(torch.exp(pred / temperature)) == 'nan':
                #pred = pred / pred.max()  #除以最大值是为了解决可能的上溢出，加一个小数为了防止下溢为0
                pred = pred / torch.abs(pred).max(dim = -1, keepdim = True)[0]
            #要判断输出是否含有inf或nan
            if (torch.exp(pred / temperature) == 0).any():
                pred = torch.exp(pred / temperature) + 1e-8 #防止下溢
            else:
                pred = torch.exp(pred / temperature)
            self.isinf_nan(pred)  
            
            loss = 0
            pos = []
            
            for i in range(len(label)):
                if args.lbl_smooth != 0:
                    index_line =  label[i] > 0.9
                else:
                    index_line =  torch.nonzero(label[i]).view(-1)#找每一行的非零元素下标
                #pos_random_idx = random.sample(index_line.cpu().tolist(), 1)
                #pos_random_idx = index_line[0:1]
                if sample_mod == 'full':  #使用全部正样本，效果是不好的
                    #pos_random_idx = index_line
                    pos_random_idx = obj[i]
                elif sample_mod == 'part':
                    #pos_random_idx = index_line
                    #pos_random_idx = index_line[0:1]
                    #pos_random_idx = random.sample(index_line.cpu().tolist(), pos_sample_num)
                    pos_random_idx = obj[i] #obj存放的是每一个三元组的真实尾节点id，这里因为只取一个正样本，所以该样本来自于obj
                    #pos_random_idx = index_line #全取试试,全取效果不好
                else:
                    exit('please gain args.sample_mod')

                rs = pred[i][pos_random_idx].sum()#对每一行加和。根据经验，加法运算不合适，因为不同的样本，正样本的数量不同，如果使用加法则导致损失忽高忽低，难以有效指导模型
                #rs = pred[i][pos_random_idx].mean()
                #pos.append(rs.view(-1)) #shape = [1]
                pos.append(rs) #shape = [1]

            pos = torch.stack(pos, dim=0)#注意使用stack处理有梯度的张量列表时不会丢失梯度，其他链接方法，如直接将列表转化成张量，会丢失梯度的shape = [B,1] 

            

            neg = []
            index_line_bool = label <= 0
            for i in range(len(label)):
                if args.lbl_smooth != 0: #如果标签平滑，则原来的0，现在 < 1，原来的1，现在 > 2
                    index_line =  label[i] < 0.1
                else:
                    index_line = torch.nonzero(index_line_bool[i]).view(-1)
                #neg_random_idx = random.sample(index_line.cpu().tolist(), 100) #减少负样本的数量，可以避免溢出
                #neg_random_idx = index_line[0:100]
                if sample_mod == 'full':
                    neg_random_idx = index_line
                    #neg_random_idx = np.random.permutation(index_line.cpu().numpy())#打乱数据，重新排列，并返回新的数据
                    
                    #index_line = index_line.cpu().numpy()
                    #np.random.shuffle(index_line) #速度更快，直接改变原数据
                    #neg_random_idx = index_line 
                elif sample_mod == 'part':
                    #neg_random_idx = index_line[0:100]
                    neg_random_idx = random.sample(index_line.cpu().tolist(), neg_sample_num) 
                    #neg_random_idx = index_line #负样本全要
                else:
                    exit('please gain args.sample_mod')

                rs = pred[i][neg_random_idx].sum()
                #rs = pred[i][neg_random_idx].mean()          
                neg.append(rs)

            neg = torch.stack(neg, dim=0)         


            if self.isinf_nan_if((- torch.log(pos / (pos + neg) )).mean()) == 'inf' or self.isinf_nan_if((- torch.log(pos / (pos + neg) )).mean()) == 'nan':
                pos = pos + 1e-8 #对数函数对接近0的数运算时，结果为-inf
                neg = neg + 1e-8   
            loss = (- torch.log(pos / (pos + neg) )).mean()  #只有这里才可能出现inf之间的运算，进而出nan    
    
            min_loss = - torch.log(pos / (pos + neg))  #只有这里才可能出现inf之间的运算，进而出nan
            max_loss = (pos / (pos + neg)) * 10000  #放大，要不然小数值太小了

            #loss = (- torch.log(pos / (pos + neg) )).sum()  #
            self.isinf_nan(loss)
        except Exception as e:
            print(e)
            #temperature = temperature + 0.1 #如果出现nan，则扩大温度系数
            #logger.info(f'temperature:{temperature}')
            exit('error')
            #continue
            
        en = time.time()
        #print('CL time:{}S'.format(en - st))    
        return min_loss.detach().cpu().tolist(), max_loss.detach().cpu().tolist()






    def analy_evaluate(self, split):
        """
        Function to evaluate the model on validation or test set

        Parameters
        ----------
        split: (string) If split == 'valid' then evaluate on the validation set, else the test set
        epoch: (int) Current epoch count

        Returns
        -------
        resutls:			The evaluation results containing the following:
            results['mr']:         	Average of ranks_left and ranks_right
            results['mrr']:         Mean Reciprocal Rank
            results['hits@k']:      Probability of getting the correct preodiction in top-k ranks based on predicted score

        """
        print('ok10')
        left_results_list, left_results, left_losses_dict = self.analy_predict(split=split, mode='tail_batch')
        print('ok11')
        right_results_list, right_results, right_losses_dict = self.analy_predict(split=split, mode='head_batch')
        print('ok12')
        results = analy_get_combined_results(left_results_list, right_results_list)
        results_mean = get_combined_results(left_results, right_results)

        losses_dict = copy.deepcopy(left_losses_dict)
        for n in right_losses_dict:
            for ls in right_losses_dict[n]:
                loss_list = right_losses_dict[n][ls]
                losses_dict[n][ls].extend(loss_list)

        return results, results_mean, losses_dict


    def cosine_similarity(self, matrix_a, matrix_b):
        # 计算矩阵A和矩阵B的余弦相似度
        dot_product = torch.mm(matrix_a, matrix_b.t())  # 点积
        norm_a = torch.norm(matrix_a, dim=1, keepdim=True)  # 矩阵A每行的模
        norm_b = torch.norm(matrix_b, dim=1, keepdim=True)  # 矩阵B每行的模

        # 计算余弦相似度
        cosine_sim = dot_product / (norm_a * norm_b.t())

        return cosine_sim

    def analy_emb(self, subs_list, rels_list, objs_list, all_ent, all_rel, labels_list):
        #度量正负节点对距离的方法，可以使用向量内积或向量减法再求长度

        #向量减法：torch.norm(obj_emb.unsqueeze(1) - all_ent, p=1, dim=2)
        #向量内积：torch.mm(obj_emb, all_ent.transpose(1, 0))

        

        my_partial_function = partial(lambda: defaultdict(  #neg or pos
                                list)) # distance)  #pickle无法打包lambda函数，因此，需要把lamada转化成函数


        #my_dict = {'neg': [], 'pos': []}

        '''
        emb_dict = defaultdict( #emb mothod transe or distmult
                    lambda: defaultdict(  #neg or pos
                                list # distance
                            ))

        '''
        emb_dict = defaultdict(my_partial_function)

        for bt in range(len(labels_list)): #处理每一个批量
            subs   = subs_list[bt]
            rels   = rels_list[bt]
            objs   = objs_list[bt]
            labels = labels_list[bt]

            sub_emb = all_ent[subs]
            rel_emb = all_rel[rels]

            transe_obj_emb   = sub_emb + rel_emb
            distmult_obj_emb = sub_emb * rel_emb
            label = copy.deepcopy(labels)

            #transe_pred   = torch.norm(transe_obj_emb.unsqueeze(1) - all_ent, p=1, dim=2) #平移不用预处理了
            #transe_pred   = torch.norm(sub_emb.unsqueeze(1) - all_ent, p=1, dim=2)

            distmult_pred1 = torch.mm(sub_emb, all_ent.transpose(1, 0))
            #distmult_pred2 = torch.sigmoid(torch.mm(distmult_obj_emb, all_ent.transpose(1, 0)))
            distmult_pred2 = torch.mm(transe_obj_emb, all_ent.transpose(1, 0)) #假设三元组满足向量加法原理。直接使用内积的相似性度量正负节点对的距离不合理，相似性与距离还不一样


            #直接向量单位化求余弦值,结果还是不行，度量方法有问题，可能需要借助主成分分析，可视化了，暂时不管了
            #cosine_similarity_matrix = self.cosine_similarity(transe_obj_emb, all_ent)
            #distmult_pred2 = cosine_similarity_matrix

            #transe_pred   = torch.norm(sub_emb.unsqueeze(1) - all_ent, p=1, dim=2) #如果必须，可能需要torch.sigmoid(x)或softmax，否则不直观
            #distmult_pred = torch.mm(sub_emb, all_ent.transpose(1, 0))

            #print('transe_pred.shape:', transe_pred.shape)
            #print('distmult_pred.shape:', distmult_pred.shape)
            #print('labels.shape:', labels.shape)
            #print('subs.shape:', subs.shape)

            for (name, pred) in zip(['distmult1', 'distmult2'], [distmult_pred1, distmult_pred2]): #这个过程是缓慢的,尽量使用GPU加速
                #print('name:', name)
                pos = []
                for i in range(len(label)):
                    if args.lbl_smooth != 0:
                        raise Exception('args.lbl_smooth != 0')
                    else:
                        index_line_all =  torch.nonzero(label[i]).view(-1)
                        #index_line = objs[i]
                        index_line = index_line_all
                        #print('label[i].shape:', label[i].shape)
                        #print('torch.nonzero(label[i]).view(-1).shape:', torch.nonzero(label[i]).view(-1).shape)
                        #print('objs[i]:', objs[i].item())

                        #if index_line not in index_line_all:
                            #raise Exception('index_line not in index_line1.tolist()')
                            
                        #raise Exception('stop')
                        pos_random_idx = index_line
                        rs = pred[i][pos_random_idx].mean()
                        pos.append(rs)
                pos = torch.stack(pos, dim=0).detach().cpu().tolist()


                neg = []
                index_line_bool = label <= 0
                for i in range(len(label)):
                    index_line = torch.nonzero(index_line_bool[i]).view(-1)
                    neg_random_idx = index_line
                    rs = pred[i][neg_random_idx].mean()
                    neg.append(rs)
                neg = torch.stack(neg, dim=0).detach().cpu().tolist()

                emb_dict[name]['neg'].extend(neg)
                emb_dict[name]['pos'].extend(pos)

        print('emb ok?')
        new_emb_dict = copy.deepcopy(emb_dict)
        for i in emb_dict:
            for j in emb_dict[i]:
                new_emb_dict[i][j] = np.fromiter(emb_dict[i][j], dtype=np.float32)  #把list转化成numpy数组
                print(f'new_emb_dict[{i}][{j}].shape:', new_emb_dict[i][j].shape)

        return new_emb_dict



    def analy_predict(self, split='valid', mode='tail_batch'):
        """
        Function to run model evaluation for a given mode

        Parameters
        ----------
        split: (string) 	If split == 'valid' then evaluate on the validation set, else the test set
        mode: (string):		Can be 'head_batch' or 'tail_batch'

        Returns
        -------
        resutls:			The evaluation results containing the following:
            results['mr']:         	Average of ranks_left and ranks_right
            results['mrr']:         Mean Reciprocal Rank
            results['hits@k']:      Probability of getting the correct preodiction in top-k ranks based on predicted score

        """
        self.model.eval()

        with torch.no_grad():
            results = {}
            results_list = defaultdict(list) #用了存放各子的结果，不求和

            losses_dict  = defaultdict( 
                    lambda: defaultdict(  
                                list 
                            ))
            
            train_iter = iter(self.data_iter['{}_{}'.format(split, mode.split('_')[0])])
            labels = []
            subs, rels, objs = [], [], []
            for step, batch in enumerate(train_iter):
                sub, rel, obj, label = self.read_batch(batch, split)

                '''
                labels.append(label.detach().cpu())
                subs.append(sub.detach().cpu())
                rels.append(rel.detach().cpu()) 
                objs.append(obj.detach().cpu())
                '''

                #labels.append(label)
                #subs.append(sub)
                #rels.append(rel) 
                #objs.append(obj)


                pred, cl_loss, all_ent, all_rel = self.model.forward(sub, rel)

                #下面的用于在给出嵌入的基础上，通过其它打分函数，求对比损失值，对比损失函数可以用于度量正负样本之间的距离
                transe_pred   = self.transe(all_ent, all_rel, sub, rel)
                distmult_pred = self.distmult(all_ent, all_rel, sub, rel)
                complex_pred  = self.complex(all_ent, all_rel, sub, rel)
                simple_pred   = self.simple(all_ent, all_rel, sub, rel)


                for (p, n) in zip([transe_pred, distmult_pred, complex_pred, simple_pred], ['transe', 'distmult', 'complex', 'simple']):

                    min_loss, max_loss = self.analy_contrastive_loss(p, label, obj)
                    
                    losses_dict[n]['min_loss'].extend(min_loss)
                    losses_dict[n]['max_loss'].extend(max_loss)

                
                b_range = torch.arange(pred.size()[0], device=self.device)
                target_pred = pred[b_range, obj]
                pred = torch.where(label.byte(), -torch.ones_like(pred) * 10000000, pred)
                pred[b_range, obj] = target_pred
                ranks = 1 + torch.argsort(torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)[
                    b_range, obj]

                ranks = ranks.float()


                results_list['count'].append(torch.numel(ranks)) 
                results_list['mr'].extend(ranks.tolist())
                results_list['mrr'].extend((1.0 / ranks).tolist())
                for k in range(10):
                    results_list['hits@{}'.format(k + 1)].extend([torch.numel(rank[rank <= (k + 1)]) for rank in ranks])

                    

                #求和
                results['count'] = torch.numel(ranks) + results.get('count', 0.0) #批量大小
                results['mr'] = torch.sum(ranks).item() + results.get('mr', 0.0)
                results['mrr'] = torch.sum(1.0 / ranks).item() + results.get('mrr', 0.0)
                #print('ranks.shape:', ranks.shape) #512
                #print("results['count'] num:", results['count']) #512
                #raise Exception('stop')
                for k in range(10):
                    results['hits@{}'.format(k + 1)] = torch.numel(ranks[ranks <= (k + 1)]) + results.get(
                        'hits@{}'.format(k + 1), 0.0)

                #if step % 100 == 0:
                    #logger.info('[{}, {} Step {}]\t{}'.format(split.title(), mode.title(), step, self.p.name))
                    #title()首字母大写,这里的输出没意义
            
            for k in results_list:
                results_list[k] = np.array(results_list[k])

            return results_list, results, losses_dict




    def gen_testdata(self): #测试集可以单独拿来用，不用再调用fit()
        #用于分析测试，保存节点和关系嵌入用于可视化，保存MRR，H@1, H@3, H@10的值(而非仅仅均值)用于显著性分析，均值、方差，中位数。pearson,spearman,T检验度量均值，F检验度量方差
        self.logger.info('Loading best model, Evaluating on Test data')
        save_path = os.path.join(args.model_dir, args.save_path)
        self.load_model(save_path)
        print('ok1')
        test_results, test_results_mean, losses_dict = self.analy_evaluate('test')

        test_data = Test_data()
        test_data.ent2id  = self.ent2id 
        test_data.id2ent  = self.id2ent
        test_data.rel2id  = self.rel2id
        test_data.id2rel  = self.id2rel
        #test_data.rel_emb = all_rel.detach().cpu().numpy()
        #test_data.ent_emb = all_ent.detach().cpu().numpy()
        test_data.results = test_results
        test_data.results_mean = test_results_mean

        print('ok2')

        #嵌入分析, 使用对比损失作为度量
        test_data.losses_dict = losses_dict

        
        test_path = os.path.join(args.analy_dir, args.analy_data, args.analy_special_test)
        print('test_path:', test_path)
        os.makedirs(test_path, exist_ok = True)

        test_path += '/' + args.analy_name

        serialized_data = dill.dumps(test_data) #对于不直接支持的打包的数据结构，需要先将其打包成字符串，然后再按二进制保存
        with open(f'{test_path}.pkl', 'wb') as file:
            file.write(serialized_data)

    
        self.logger.info('\n\n')
        self.logger.info('测试:')
        self.logger.info(('Test_mrr: %.5f') % test_results_mean['mrr'])
        self.logger.info(('Test_mr: %.5f') % test_results_mean['mr'])
        self.logger.info(('Test_hit1: %.5f') % test_results_mean['hits@1'])
        self.logger.info(('Test_hit3: %.5f') % test_results_mean['hits@3'])
        self.logger.info(('Test_hit10: %.5f') % test_results_mean['hits@10'])
        localtime = time.asctime(time.localtime(time.time()))
        self.logger.info(f'程序终止时间:{localtime}')

        return test_results_mean

    def plot(self):
        pass

    def analy(self):
        test_path = os.path.join(args.analy_dir, args.analy_data, args.analy_special_test)
        print('test_path:', test_path)
        os.makedirs(test_path, exist_ok = True)

        test_path += '/' + args.analy_name

        print('test_path:', test_path)

        with open(f'{test_path}.pkl', 'rb') as file:
            loaded_data = file.read()
            data = dill.loads(loaded_data)
        
        test_results = data.results
        test_results_mean = data.results_mean

        w_f = open(f'{test_path}_result.txt', 'w')

        w_f.write('Mean:\n')
        w_f.write(('Test_mrr: %.5f\n') % (np.mean(test_results['mrr'])))
        w_f.write(('Test_mr: %.5f\n') % (np.mean(test_results['mr'])))
        w_f.write(('Test_hit1: %.5f\n') % (np.mean(test_results['hits@1'])))
        w_f.write(('Test_hit3: %.5f\n') % (np.mean(test_results['hits@3'])))
        w_f.write(('Test_hit10: %.5f\n\n') % (np.mean(test_results['hits@10'])))



        w_f.write('Std:\n')
        w_f.write(('Test_mrr: %.5f\n') % (np.std(test_results['mrr'])))
        w_f.write(('Test_mr: %.5f\n') % (np.std(test_results['mr'])))
        w_f.write(('Test_hit1: %.5f\n') % (np.std(test_results['hits@1'])))
        w_f.write(('Test_hit3: %.5f\n') % (np.std(test_results['hits@3'])))
        w_f.write(('Test_hit10: %.5f\n\n') % (np.std(test_results['hits@10'])))



        w_f.write('Median:\n')
        w_f.write(('Test_mrr: %.5f\n') % (np.median(test_results['mrr'])))
        w_f.write(('Test_mr: %.5f\n') % (np.median(test_results['mr'])))
        w_f.write(('Test_hit1: %.5f\n') % (np.median(test_results['hits@1'])))
        w_f.write(('Test_hit3: %.5f\n') % (np.median(test_results['hits@3'])))
        w_f.write(('Test_hit10: %.5f\n\n') % (np.median(test_results['hits@10'])))


        print('测试1:')
        print(('Test_mrr: %.5f') % (np.mean(test_results['mrr'])))
        print(('Test_mr: %.5f') % (np.mean(test_results['mr'])))
        print(('Test_hit1: %.5f') % (np.mean(test_results['hits@1'])))
        print(('Test_hit3: %.5f') % (np.mean(test_results['hits@3'])))
        print(('Test_hit10: %.5f') % (np.mean(test_results['hits@10'])))

        print('\n')

        
        print('测试2:')
        count = test_results['count'].sum()
        print(('Test_mrr: %.5f') % (np.sum(test_results['mrr']) / count))
        print(('Test_mr: %.5f') % (np.sum(test_results['mr']) / count))
        print(('Test_hit1: %.5f') % (np.sum(test_results['hits@1']) / count))
        print(('Test_hit3: %.5f') % (np.sum(test_results['hits@3']) / count))
        print(('Test_hit10: %.5f') % (np.sum(test_results['hits@10']) / count))
        

        print('\n')

        print('测试3:')
        print(('Test_mrr: %.5f') % test_results_mean['mrr'])
        print(('Test_mr: %.5f') % test_results_mean['mr'])
        print(('Test_hit1: %.5f') % test_results_mean['hits@1'])
        print(('Test_hit3: %.5f') % test_results_mean['hits@3'])
        print(('Test_hit10: %.5f\n\n') % test_results_mean['hits@10'])
        

        


        #度量正负节点对距离的方法，可以使用向量内积或向量减法再求长度
        #向量减法：torch.norm(obj_emb.unsqueeze(1) - all_ent, p=1, dim=2)
        #向量内积：torch.mm(obj_emb, all_ent.transpose(1, 0))
        #数据结构 losses_dict['transe']['min_loss']
        losses_dict = data.losses_dict   

        for f in losses_dict:
            print(('measure emb method: %s') % (f))
            w_f.write(('measure emb method: %s\n') % (f))
            for k in losses_dict[f]:
                print(f'{k}')
                w_f.write(f'{k}\n')
                print('len(emb_dict[f][k]):', len(losses_dict[f][k]))  #应该等于60422 * 2
                sum    = np.sum(losses_dict[f][k])
                mean   = np.mean(losses_dict[f][k])
                std    = np.std(losses_dict[f][k])
                median = np.median(losses_dict[f][k])

                print(('sum: %.5f\n') % (sum))
                print(('mean: %.5f') % (mean))
                print(('std: %.5f') % (std))
                print(('median: %.5f\n\n') % (median))

                w_f.write(('sum: %.5f\n') % (sum))
                w_f.write(('mean: %.5f\n') % (mean))
                w_f.write(('std: %.5f\n') % (std))
                w_f.write(('median: %.5f\n\n') % (median))



        w_f.close()
        return test_results_mean





# 定义一个自定义类
class Test_data(object):
    #自定义一个数据类用于保存测试中生成的数据集
    def __init__(self):
        self.ent2id = {}
        self.id2ent = {}
        self.rel2id  = {}
        self.id2rel  = {}
        self.rel_emb = None
        self.ent_emb = None
        self.results = defaultdict(list)  #存放各个指标
        self.results_mean  = {} #存放各个指标的均值
        self.labels = None
        self.emb_dict = {}
        self.losses_dict = {} #将对比损失作为度量正负节点距离的方法



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Parser For Arguments', formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('--name',	    default='rhcgat',					help='Set run name for saving/restoring models')
    parser.add_argument('--dataset',	dest='dataset',         default='FB15k-237',            help='Dataset to use, default: FB15k-237')
    parser.add_argument('--data_name',  default='fb15k',        type=str,                       help='The data name of preprocessed graph.')
    parser.add_argument('--model',	    dest='model',		    default='rhcgat',		        help='Model Name')
    parser.add_argument('--score_func',	dest='score_func',	    default='conve',		        help='Score Function for Link prediction')
    parser.add_argument('--opn',        dest='opn',             default='corr',                 help='Composition Operation to be used in CompGCN')

    parser.add_argument('--batch_size', dest='batch_size',      default=512,    type=int,       help='Batch size for GAT')
    parser.add_argument('--gamma',	    type=float,             default=40,			            help='Margin, default = 40')
    parser.add_argument('--gpu',	    type=str,               default='0',			        help='Set GPU Ids : Eg: For CPU = -1, For Single GPU = 0')
    parser.add_argument('--epoch',	    dest='max_epochs', 	    type=int,       default=1000,  	help='Number of epochs')
    parser.add_argument('--l2',		    type=float,             default=0.0,			        help='L2 Regularization for Optimizer')
    parser.add_argument('--lr',		    type=float,             default=0.001,			        help='Starting Learning Rate')
    parser.add_argument('--lbl_smooth', dest='lbl_smooth',	    type=float,     default=0,	    help='Label Smoothing') #默认0.1，使用对比损失，必须设置为0
    parser.add_argument('--num_workers',type=int,               default=10,                     help='Number of processes to construct batches')
    parser.add_argument('--seed',       dest='seed',            default=41504,  type=int,     	help='Seed for randomization')

    #parser.add_argument('--restore',   dest='restore',         action='store_true',            help='Restore from the previously saved model')
    parser.add_argument('--bias',       dest='bias',            default=0,      type=int,       help='Whether to use bias in the model')
    #parser.add_argument('--hid_drop',   dest='hid_drop',        default=0.3, type=float, help='Dropout after GCN')
    parser.add_argument('--num_bases',	dest='num_bases', 	    default=-1,   	type=int, 	    help='Number of basis relation vectors to use')
    parser.add_argument('--init_dim',	dest='init_dim',	    default=100,	type=int,	    help='Initial dimension size for entities and relations')
    parser.add_argument('--gcn_dim',	dest='gcn_dim', 	    default=200,   	type=int, 	    help='Number of hidden units in GCN')
    parser.add_argument('--embed_dim',	dest='embed_dim', 	    default=200,    type=int, 	    help='Embedding dimension to give as input to score function')
    parser.add_argument('--gcn_layer',	dest='gcn_layer', 	    default=2,   	type=int, 	    help='Number of GCN Layers to use')
    parser.add_argument('--gcn_drop',	dest='dropout', 	    default=0.1,  	type=float,	    help='Dropout to use in GCN Layer')


    # arguments for GAT.不要使用bool类型了，作用和action='store_true',一样，如果指定值，则为真，否则使用默认值。使用0与1最好
    parser.add_argument('--pretrained_emb',         dest='pretrained_emb',      default=0, type=int, help='')
    parser.add_argument('--num_of_layers',          dest='num_of_layers',       default=2, type=int, help='')
    parser.add_argument('--num_heads_per_layer',    dest='num_heads_per_layer', nargs="*", default=[8, 1], type=int, help='')#两层网络，每一层的注意力头数量
    parser.add_argument('--add_skip_connection',    dest='add_skip_connection', default=0, type=int, help='')
    parser.add_argument('--bias_gat',               dest='bias_gat',            default=1, type=int, help='')
    parser.add_argument('--dropout_gat',            dest='dropout_gat',         default=0.5, type=float, help='')
    parser.add_argument('--num_features_per_layer', dest='num_features_per_layer', nargs="*",
                        default=[200, 25, 200], type=int, help='') #25是每一个注意力头的维度，即200/8 = 25
    
    #改一下注意力头和网络层的执行，要更灵活，而不是只能执行2层网络。

    # ConvE specific hyperparameters
    parser.add_argument('--hid_drop',   dest='hid_drop',    default=0.3,    type=float,     help='ConvE1: Hidden dropout')
    parser.add_argument('--hid_drop2',  dest='hid_drop2',   default=0.4,  	type=float,	    help='ConvE2: Hidden dropout')
    parser.add_argument('--feat_drop', 	dest='feat_drop',   default=0.3,  	type=float,	    help='ConvE: Feature Dropout')
    parser.add_argument('--k_w',	    dest='k_w', 	    default=10,   	type=int, 	    help='ConvE: k_w')
    parser.add_argument('--k_h',	    dest='k_h', 	    default=20,   	type=int, 	    help='ConvE: k_h')
    parser.add_argument('--num_filt',  	dest='num_filt',    default=200,   	type=int, 	    help='ConvE: Number of filters in convolution')
    parser.add_argument('--ker_sz',    	dest='ker_sz', 	    default=8,   	type=int, 	    help='ConvE: Kernel size to use')

    #parser.add_argument('--logdir',          dest='log_dir',         default='./log/',               help='Log directory')
    #parser.add_argument('--config',          dest='config_dir',      default='./config/',            help='Config directory')


    #一些新参数
    parser.add_argument('--pos_sample_num',       type=int,   default=1,    help='positive sample num')
    
    parser.add_argument('--neg_sample_num',       type=int,   default=100,  help='negtive sample num')

    #设置2种损失函数函数的比重
    parser.add_argument('--subgraph_loss_rate',     type=float, default=0.0, help='loss_rate')   #默认0.001

    parser.add_argument('--node_loss_rate',         type=float, default=0.001, help='loss_rate') #目前没用

    parser.add_argument('--local_rank',     default=0,  type=int,   help='node rank for distributed training')
    
    parser.add_argument('--use_pretrain',   type=int,   default=0,  help='whether use pretrain')

    parser.add_argument('--use_test',       type=int,   default=0,  help='whether be used in test')

    parser.add_argument('--test',           type=int,   default=0,  help='whether be used in testdata') #配合save_path使用，仅用于测试数据集

    parser.add_argument('--test_name',      type=str,   default='', help='test name')

    parser.add_argument('--model_dir',      type=str,   default='./fb15k237_model_save', help='The address for storing the models and optimization results.')

    parser.add_argument('--task_name',      type=str,   default='LP', help='The name of the stored models and optimization results.')

    parser.add_argument('--log_dir',         dest='log_dir',     default='./fb15k237_log/',   help='Log directory')

    parser.add_argument('--config',         dest='config_dir',  default='./config/',        help='Config directory')
            
    parser.add_argument('--loss_function',  type=str,   default='bcel_loss',
                        choices=['contrastive_loss', 'mask_softmax', 'mix_loss','bcel_loss', 'magr_loss'],
                        help='choose loss function')#改一下,多标签损失

    parser.add_argument('--mix_loss_mod',  type=str,   default='CLoss-MLoss',
                        choices=['CLoss-MLoss', 'CLoss-BCELoss', 'MLoss-BCELoss'],
                        help='choose loss function')#改一下,多标签损失

    parser.add_argument('--sample_mod',     type=str,   default='full',     help='对比学习的全部采样和部分采样')
    
    parser.add_argument('--stop_num',       type=int,   default=50,   help='stop num of epoch')

    parser.add_argument('--save_path',      type=str,   default=None,     help='load model from save path')

    parser.add_argument('--optimizer',      type=str,   default='adam',
                        choices=['adamw', 'adam', 'sgd', 'adagrad'],#学习率的优化，注意所给的学习率优化器必须在choice里面
                        help='optimizer to use.')
    
    parser.add_argument('--change_lr',      default=0,  type=int,   help='change lr') #优先使用动态调整学习率的方法，找到最优之后再使用静态的学习率

    parser.add_argument('--lr_scheduler',   type=str,   default='CosineAnnealingLR',
                        choices=['CyclicLR', 'OneCycleLR', 'CosineAnnealingWarmRestarts', 'ReduceLROnPlateau', 'CosineAnnealingLR'],#学习率的优化，注意所给的学习率优化器必须在choice里面
                        help='lr scheduler to use.')
    
    parser.add_argument('--clip',           type=float, default=0.25,   help='Gradient Norm Clipping') #梯度修剪，防止梯度爆炸吗？是的

    parser.add_argument('--workdir',        type=str, default='GNN',    help='Gradient Norm Clipping')
    
    parser.add_argument('--conv_name', type=str, default='cmgat',#根据不同的模型名，执行不同的模型,这个看看如何切换
                        choices=['rhcgat', 'rhcgcn', 'rahcgt', 'transe', 'distmult', 'complex', 'simple',\
                                 'origin_sage', 'lgtnv3', 'lgtnv2', 'lgtn', 'rgcnv3', 'rgcnv2', 'rel_dense_mthgcl2', 'gatv4',\
                                 'gatv3', 'hrgat', 'heat', 'eg', 'film', 'gcnv2', 'gatv2', 'transformer', 'fa', 'supergat',\
                                 'indrel_dense_mthgcl', 'rel_dense_mthgcl', 'norel_dense_mthgcl', 'mthgcl', 'dense_mthgcl',\
                                 'mtmrhgnn', 'dense_mtmrhgnn', 'hgcl', 'dense_hgcl', 'compgcn', 'sage', 'hgt', 'gcn', 'gat', 'rgcn',\
                                 'han', 'hetgnn', 'sagecn'],
                        help='The name of GNN filter. By default is Heterogeneous Graph Transformer (hgt)')
    
    #relu, gelu, tanh, leakyrelu, selu, hardswish, relu6
    parser.add_argument('--conve_act',       type=str, default = 'relu6', choices = ['relu', 'gelu', 'tanh', 'leakyrelu', 'selu', 'hardswish', 'relu6'],
        help='选择conve的激活函数, 对比损失适合relue6,多标签分类适合relu')

    '''
    HCL的对比损失参数

    '''
    parser.add_argument('--temperature',    default=0.2,    type=float, help='Temperature used in softmax')
    parser.add_argument('--tau_plus',       default=0.1,    type=float, help='Positive class priorx')
    parser.add_argument('--beta',           default=0.6,    type=float, help='concentration parameter')
    parser.add_argument('--estimator',      default='easy', type=str,   help='Choose loss function')
    parser.add_argument('--Q',              default=1.0,    type=float, help='pso and neg sample of rate') #
    
    parser.add_argument('--change_tem',     default=1,      type=int,   help='whether change tem')
    
    parser.add_argument('--special_test',   default=None,   type=str,   help='whether use special test data')



    #测试参数
    parser.add_argument('--analy_dir',    default='Test',   type=str,   help='')
    parser.add_argument('--analy_data',   default='data',   type=str,   help='')
    parser.add_argument('--analy_special_test',   default='',   type=str,   help='whether use special test data')
    parser.add_argument('--analy_name',   default='',     type=str,   help='')
    parser.add_argument('--store_emb',    default=None,   type=str,   help='save embedding')
    parser.add_argument('--analy',        default=0,      type=int,   help='')


    args = parser.parse_args()
    
    #logger = get_logger( 'stdout.log', args.workdir, args.config_dir) #新方法为什么依旧没有标准输出呀？
    #logger.setLevel(logging.DEBUG)
    
    os.makedirs(args.model_dir, exist_ok = True)
    os.makedirs(args.log_dir, exist_ok = True)

    if args.analy_special_test:
        tg_dir = os.path.join(args.dataset, args.analy_special_test)
        tg_dir = '../data/' + tg_dir
        file_list = []
        files = os.listdir(tg_dir)
        for f in files:
            if '.txt' in f:
                tg = os.path.splitext(f)[0]
                file_list.append(tg)

        test_path = os.path.join(args.analy_dir, args.analy_data)
        print('test_path:', test_path)
        os.makedirs(test_path, exist_ok = True)

        sub_w = open(f'{test_path}/{args.analy_name}_sub_test_results.txt', 'w')
    
        for i in file_list:
            print('--------------------------------------------------------')
            print('current file:', i)

            args.analy_special_test = i
            args.special_test = i

            if args.use_pretrain:
                if args.use_test:
                    name = args.test_name + '_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
                else:
                    name = args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
            else:
                if args.use_test:
                    name = args.test_name + '_' + 'notext_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
                else:
                    name = 'notext_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')

            #args.name = name#新增参数
            

            args.name = name#新增参数

            set_gpu(args.gpu)
            np.random.seed(args.seed)
            torch.manual_seed(args.seed)
            
            if args.save_path != None:
                args.temperature = args.temperature
            else:
                args.temperature = args.temperature
            
            '''
            args.file_name = file_name         = time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S') 
            if args.use_test:
                #args.workdir  = 'workdir/' + args.test_name + '_' + file_name 
                args.workdir  = 'workdir/' + args.task_name + '_'+ args.name
            else:
                #args.workdir  = 'workdir/' + file_name 
                args.workdir  = 'workdir/' + args.task_name + '_'+ args.name

            #tf.io.gfile.makedirs(args.workdir) #不存在，则创建目录,tf阻碍logging屏幕输出

            if not os.path.isdir(args.workdir):
                os.makedirs(args.workdir)
            
            '''

            dist.init_process_group(backend="nccl")
            model = Runner(args)

            if args.test != 1:
                model.fit() #训练，验证和测试一体
            else: 
                #model.test()#仅用于测试数据,需要配合args.save_path来使用
                #test_results_mean = model.analy()

                if args.analy == 1:
                    test_results_mean = model.analy()  #对测试数据分析
                else:
                    #model.test()#仅用于测试数据,需要配合args.save_path来使用
                    model.gen_testdata() #生成并保存测试数据
                    test_results_mean = model.analy()
                

            
            #记录子集信息：

            sub_w.write(('current file: %s\n') % i)
            sub_w.write(('Test_mrr: %.5f\n') % test_results_mean['mrr'])
            sub_w.write(('Test_mr: %.5f\n') % test_results_mean['mr'])
            sub_w.write(('Test_hit1: %.5f\n') % test_results_mean['hits@1'])
            sub_w.write(('Test_hit3: %.5f\n') % test_results_mean['hits@3'])
            sub_w.write(('Test_hit10: %.5f\n') % test_results_mean['hits@10'])



            dist.destroy_process_group()
            
            current_directory = os.path.dirname(os.path.abspath('__file__')) #获取当前路径
            pt = os.path.join(current_directory, args.log_dir.replace('./', ''), args.task_name + '_' + args.name + '.log')
            logging.info(('file_path: %s') % (pt)) #这个语句不会在标准输出上显示，但是会被写入文件
            print('file_path:', pt)

            sub_w.write(('file_path: %s\n\n\n') % (pt))
            
            localtime = time.asctime(time.localtime(time.time()))
            logging.info(f'程序终止时间:{localtime}')

        sub_w.close()
    
    else:
        if args.use_pretrain:
            if args.use_test:
                name = args.test_name + '_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
            else:
                name = args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
        else:
            if args.use_test:
                name = args.test_name + '_' + 'notext_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')
            else:
                name = 'notext_' + args.score_func + '_' + args.loss_function + '_' + args.dataset + '_'+ 'CL'+ args.conv_name + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S')

        #args.name = name#新增参数
        

        args.name = name#新增参数

        set_gpu(args.gpu)
        np.random.seed(args.seed)
        torch.manual_seed(args.seed)
        
        if args.save_path != None:
            args.temperature = args.temperature
        else:
            args.temperature = args.temperature
        
        '''
        args.file_name = file_name         = time.strftime('%d_%m_%Y') + '_' + time.strftime('%H_%M_%S') 
        if args.use_test:
            #args.workdir  = 'workdir/' + args.test_name + '_' + file_name 
            args.workdir  = 'workdir/' + args.task_name + '_'+ args.name
        else:
            #args.workdir  = 'workdir/' + file_name 
            args.workdir  = 'workdir/' + args.task_name + '_'+ args.name

        #tf.io.gfile.makedirs(args.workdir) #不存在，则创建目录,tf阻碍logging屏幕输出

        if not os.path.isdir(args.workdir):
            os.makedirs(args.workdir)
        
        '''

        dist.init_process_group(backend="nccl")
        model = Runner(args)

        if args.test != 1:
            model.fit() #训练，验证和测试一体
        else: 
            if args.analy == 1:
                model.analy()  #对测试数据分析
            else:
                #model.test()#仅用于测试数据,需要配合args.save_path来使用
                model.gen_testdata() #生成并保存测试数据
                model.analy()
            
            
        dist.destroy_process_group()
        
        current_directory = os.path.dirname(os.path.abspath('__file__')) #获取当前路径
        pt = os.path.join(current_directory, args.log_dir.replace('./', ''), args.task_name + '_' + args.name + '.log')
        logging.info(('file_path: %s') % (pt)) #这个语句不会在标准输出上显示，但是会被写入文件
        print('file_path:', pt)
        
        localtime = time.asctime(time.localtime(time.time()))
        logging.info(f'程序终止时间:{localtime}')
    


