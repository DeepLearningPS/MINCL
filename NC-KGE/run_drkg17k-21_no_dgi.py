    ...
    def fit(self):
        ...
        for epoch in np.arange(self.p.max_epochs) + 1: 
            ...
            if val_results['mrr'] > best_val:
                ...
                if kill_cnt % 10 == 0 and self.p.save_path != None: #加载模型，温度系数需要递减
                    if self.p.temperature >= 1.0:
                        self.p.temperature = 0.05
                    else:
                        self.p.temperature += 0.05 #增加温度系数值,
                    self.logger.info('temperature add on saturation, updated value of temperature {}'.format(self.p.temperature))
                    self.logger.info('\n\n')
                    
                if kill_cnt % 10 == 0 and self.p.save_path == None: #从头训练，温度系数需要递增
                    if self.p.temperature <= 0.05:
                        self.p.temperature = 1.0
                    else:
                        self.p.temperature -= 0.05 #减少温度系数值,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',	dest='dataset',         default='DRKG17k-21',            help='Dataset to use, default: FB15k-237')
    parser.add_argument('--data_name',  default='drkg',        type=str,                       help='The data name of preprocessed graph.')
    ...
    parser.add_argument('--model_dir',      type=str,   default='./drkg17k-21_model_save', help='The address for storing the models and optimization results.')

    parser.add_argument('--task_name',      type=str,   default='LP', help='The name of the stored models and optimization results.')

    parser.add_argument('--logdir',         dest='log_dir',     default='./drkg_log/',   help='Log directory')
